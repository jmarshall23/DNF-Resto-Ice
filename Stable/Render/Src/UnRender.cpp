...
//=========================================================================================
void URender::CreateShadowRenderTargets(UViewport* Viewport, INT NumRenderTargets)
{
	if (GRender->ShadowRenderTargetsCreated)
		return;

	NumShadowRenderTargets = 0;

	//for (INT i = 0; i<MAX_SHADOW_RENDER_TARGETS; i++)
	for (INT i = 0; i < NumRenderTargets; i++)
	{
		// FairFriend: Custom shadow resolution is passed as arrguement. 		
		ShadowRenderTargets[NumShadowRenderTargets].RenderTarget = Viewport->RenDev->CreateRenderTarget(GRender->iShadowRes, GRender->iShadowRes);

		if (ShadowRenderTargets[NumShadowRenderTargets].RenderTarget)
			NumShadowRenderTargets++;
	}

	debugf(TEXT("Created %i shadowcaster render targets"), NumShadowRenderTargets);

	GRender->ShadowRenderTargetsCreated = true;
	ResetRenderTargets();
}
...
static void DestroyShadowRenderTargets(UViewport* Viewport)
{
	...
	for (INT i = 0; i<NumShadowRenderTargets; i++)
	{
		if (!ShadowRenderTargets[i].RenderTarget)
			continue;
		
		Viewport->RenDev->DestroyRenderTarget(&ShadowRenderTargets[i].RenderTarget);
		ShadowRenderTargets[i].RenderTarget = NULL;
	}
...
static void FindShadowActorLights(FSceneNode *Frame, WorkShadowActor *SActor, FPlane **Planes, INT NumPlanes)
{
	...
	for (INT j = 0; j < NumShadowLights; j++)
	{
		...
		// FairFriend: Update shadow frustrum size.
		#define SHADOW_FRUSTUM_WIDTH		(GRender->iShadowRes)
...
void BuildShadowCasterList(FSceneNode *Frame, TArray<AActor*> &Relevent)
{
	...
	while(1)
	{
		...
		for (INT i = 0; i < NumWorkShadowActorsToUse; i++)
		{
			...
			if (NumFinalShadowActors >= GRender->GMaxFinalShadowActors)
				break;
...
static void BuildShadowCasters(FSceneNode *Frame, TArray<AActor*> &Relevent)
{
	...
	for (i=0; i< NumFinalShadowActors; i++)
	{
		...
		if (Render)
		{
			...
			Frame->Viewport->RenDev->SetRenderTarget(RenderTarget, NULL);
		...
		// Add the shadowcaster to the driver
		Frame->Viewport->RenDev->AddProjector(ShadowFrame, RenderTarget, NULL, wNear, wFar, FadeScale);
	
		Actor->ProjectorFlags &= ~(1<<NumProjectors);
	...
	// Set the render target back to the way it was
	Frame->Viewport->RenDev->RestoreRenderTarget();
...
void URender::Init( UEngine* InEngine )
{
	...
	ShapeLODMode      = 1;
	ShapeLODFix       = 1.f;
	// FairFriend: Default dynamic shadows settings.
	ShadowRenderTargetsCreated = false;
	GMaxFinalShadowActors = 16;	 
	iShadowRes = 512;	
	bShadowResChanged = false;
...
void URender::Precache( UViewport* Viewport )
{
		...
		for( INT i=0; i<ItM->Surfs.Num(); i++ )
		{
			...
			if( It )
			{
				...
				if( !It->bParametric )
				{
					...
					Viewport->RenDev->PrecacheTexture( T, It->PolyFlags|ItS.PolyFlags, It->PolyFlagsEx|ItS.PolyFlags2 );
					It->Unlock( T );
				}
			}
		}
	...
	for( TObjectIterator<UTexture> It; It; ++It )
	{
		if( !It->bParametric )
		{
			FTextureInfo T;
			It->Lock( T, Viewport->LastUpdateTime, -1, Viewport->RenDev );
			Viewport->RenDev->PrecacheTexture( T, It->PolyFlags, It->PolyFlagsEx );
			It->Unlock( T );
		}
	}
}
		...
		if( Pass == PASS_Plane )
		{
			...
			for( ;; )
			{
				...
				&&	(Viewport->Actor->ShowFlags & SHOW_PlayerCtrl) )
				{
					...
					if( (PolyFlags & PF_Translucent || PolyFlagsEx & PFX_Translucent2) && !RenDev->ShinySurfaces )
					{
...
void __fastcall URender::DrawFrame( FSceneNode* Frame )
{
	...
	for( Pass=0; Pass<3; Pass++ )
	{
		...
		for( FBspDrawListPtr* DrawPtr = FirstDraw[Pass]; DrawPtr<LastDraw[Pass]; DrawPtr++ )
		{
			...
			Surface.PolyFlagsEx		= 0;
			Surface.LightMap		= NULL;
			Surface.MacroTexture	= NULL;
			Surface.DetailTexture	= NULL;
			Surface.FogMap			= NULL;	

			// JEP Pass off projector flags to the driver (this is the visible projectors from this polygon)
			//if (Surf->ProjectorFrame == GFrameStamp)
				Surface.ProjectorFlags = Surf->ProjectorFlags;
			//else
			//	Surface.ProjectorFlags = 0;
				...
				for( INT i=0; i < Surf->Decals.Num(); i++ )
				{
					...
					for( FSavedPoly* PolyIt=Facet.Polys; PolyIt; PolyIt = PolyIt->Next )
					{
						...
						if(DecalNumPoints != 0)
						{
							...
							Viewport->RenDev->DrawGouraudPolygon( Frame, DecalTextureInfo, DecalPoints, DecalNumPoints, DecalIt->PolyFlags|PF_Unlit, &Draw->Span, DecalIt->PolyFlagsEx);
		...
		for( FDynamicSprite* Sprite = Frame->Sprite; Sprite; Sprite=Sprite->RenderNext )
		{
			UBOOL bTranslucent = Sprite->Actor && (Sprite->Actor->Style==STY_Translucent || Sprite->Actor->Style == STY_Translucent2);
			if
			(	(Pass==2 && bTranslucent)
...
//				if(!(!RenderCurrent->bHighDetail || HighDetailActors)) continue; // Handle high detail bastardos 
	...
	if (Frame->Viewport->GetOuterUClient()->ShadowDetail == 1 && GMaxFinalShadowActors != 8) // FairFriend: Update settings only if need be
	{
		GMaxShadowLightsPerActor = 1;
		GMaxFinalShadowActors = 8;
		iShadowRes = 128;	// FairFriend: Dynamic shadows resolution setting
		bShadowResChanged = true;
	}
	else if (Frame->Viewport->GetOuterUClient()->ShadowDetail == 2 && GMaxFinalShadowActors != 12)
	{
		GMaxShadowLightsPerActor = 1;
	...
	else if (Frame->Viewport->GetOuterUClient()->ShadowDetail == 3 && GMaxFinalShadowActors != 16)
	{
	...
	#if 1
		// FairFriend: Destroy the render targes also if the dynamic shadows resolution has changed. 
		if (ShadowRenderTargetsCreated && (GMaxFinalShadowActors != NumShadowRenderTargets || bShadowResChanged))
...
STAT(unclock(GStat.OcclusionTime));		// JEP
	...
	if (NumShadowRenderTargets > 0)
		Frame->Viewport->RenDev->ResetProjectors();
