...
class DLL_EXPORT UD3DRenderDevice : public URenderDevice
{
	...
	void __fastcall DrawPrimitiveWithDepthPass(D3DPRIMITIVETYPE PrimitiveType, UINT StartVertex, UINT PrimitiveCount)
	{
		...
		Direct3DDevice8->DrawPrimitive(PrimitiveType, StartVertex, PrimitiveCount);
	...
	BOOL __fastcall SetZWriteEnable(BOOL NewZWriteEnable = TRUE)
	{
		...
		{
			Direct3DDevice8->SetRenderState(D3DRS_ZWRITEENABLE, NewZWriteEnable);
	...
	D3DCMPFUNC __fastcall SetZFunc(D3DCMPFUNC NewZFunc = D3DCMP_LESSEQUAL)
	{
		...
		{
			Direct3DDevice8->SetRenderState(D3DRS_ZFUNC, NewZFunc);
			ZFunc = NewZFunc;
		}
	...
	void __fastcall SetTexture( DWORD dwStage, FTextureInfo& Info, DWORD PolyFlags=0, UBOOL Precache=FALSE, DWORD PolyFlagsEx=0, UBOOL IsLightmap=FALSE )
	{
		...
		if (!TexInfo->UseMips | (PolyFlags & PF_Translucent) | (PolyFlagsEx & (PFX_AlphaMap|PFX_Translucent2)))
			Direct3DDevice8->SetTextureStageState(dwStage, D3DTSS_MIPFILTER, D3DTEXF_NONE);
		else if (!Stages[dwStage] || Stages[dwStage]->UseMips)
			Direct3DDevice8->SetTextureStageState(dwStage, D3DTSS_MIPFILTER, UseTrilinear ? D3DTEXF_LINEAR : D3DTEXF_POINT);
...
#if 1 // [JM] Is it really a good idea to disable this? Wouldn't it fuck up GetRes?
	...
	void __fastcall FlushProjectorPolys(void)
	{
		...
		for (INT p=0; p< ProjectorArray.Num(); p++)
		{
			...
			for (i=0; i< NumProjectorSurfs; i++)
			{
				...
				{
					if (PolysVisible[j])
						// Raziel note: shadow rendering in batch mode
						SetZBias(10.0f); // Raziel: Add a Z bias, it helps, but doesn't elinimate all shadow
										 // glitching, would recommend not enabling batch mode
