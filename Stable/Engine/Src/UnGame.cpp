...
void __fastcall UGameEngine::Tick( FLOAT DeltaSeconds )
{
	...
	// If game is paused, release the cursor in windowed mode.
	if
		(Client
	...
	{
		// Raziel note: GEntry represents the 'Entry.dnf' map (the main menu), and the pauser is set when the
		//  game is paused. This should cover the two scenarios when the mouse should be released...
		UBOOL IsPausedOrAtMainMenu = GLevel->GetLevelInfo()->Pauser != TEXT("") || (GEntry && GEntry == GLevel);

		if (IsPausedOrAtMainMenu)
			Client->Viewports(0)->SetMouseCapture(0, 0, 0);
		else if (!IsPausedOrAtMainMenu && Client->CaptureMouse)
