...
FLOAT* UInput::FindAxisName( AActor* Actor, const TCHAR* ButtonName ) const
{
	...
	if( Button != NAME_None )
	{
		...
		// Raziel: Often after importing new models, the FInputVarCache returned is complete garbage,
		// while the count would normally be around 28, it ends up with numbers like 100 million which
		// ends up crashing the editor. As a work-around, don't use the cache if the count is unusually
		// high. I haven't seen any side-effects of doing this, but we should try and find a proper fix.
...
#if 1		
	// (JEP) Carry key status over 
	// (was making it quickload twice when hitting F9 from the main menu)
	//	I traced it down to the fact that when it switched levels, GetKeyState was returning that the key was
	//	still pressed, but KeyDownTable had been reset, so it issued a another KeyDown message, which tried to load
	//	the level again...
	for( INT i=0; i<IK_MAX; i++ )
	{
