...
// Internal variables.
//var native private transient const int Magic;
...
//var native const int UnboundProperties;
...
// Integer functions.
native      static final function	  int  Seed  ( int Seed );
...
native		static final function	  int  Round ( float A );
...
native(207) static final function     float Atan2 ( float Y, float X );
...
native(208) static final function     rotator Slerp ( float Alpha, rotator A, rotator B );

// Vector operators.
native(204) static final function     vector vect  ( optional float X, optional float Y, optional float Z );
...
native(205) static final function     rotator rot ( optional int Pitch, optional int Yaw, optional int Roll );
...
native(198) static final function bool   IsValidString ( string S );
...
// CDH: added child state stack support
native(113) final function GotoState( optional name NewState, optional name Label ); // goto top-level parent state and destroy child stack
native(281) final function bool IsInState( name TestState ); // returns true if in the given state at any depth, not just current depth
native(284) final simulated function name GetStateName( optional int Depth ); // depth is number of parent states deep in the stack
native(199) final function ChangeState( optional name NewState, optional name Label ); // similar to gotostate, but at current child depth
native(200) final function ChildState( optional name NewState, optional name Label ); // push state position on stack, then changestate
native(201) final function EndChildState(); // called by child state, pops state position and returns to parent
native(202) final function int GetStateDepth(); // returns depth of state stack
...
native(206) static final function class ClassForName( name ClassName );
...
native static final function LogStackTrace();
