...
simulated final event vector CalcDrawOffset()
{
	...
	if ( Owner.IsA('PlayerPawn') )
	{
		...
		DrawOffset -= X*PlayerOwner.WeapShakeOffset;
		DrawOffset += PlayerOwner.VibrationVector;
		...
		// 					I KNOW IT LOOKS WEIRD BUT I KNOW WHAT IM DOING 
		//					TRUST ME I ACTUALLY TESTED THIS -x
		DrawOffset += (PlayerOwner.VibrationVector * 0.1);


		// TODO: When looking down, make weapon move somewhere -x
		//if ((65535 - ViewRotation.Pitch) < 32768)
			//CameraLocation	+= (Y+Z-X) * 0.5 * (Float(65535 - ViewRotation.Pitch) / 16384.0);
		
		// if someone decides to do #174, let me help:
		//CameraLocation += X * <RAYCASTED COLLISION DISTANCE CALCULATION GOES HERE>
		// -x
	}
...
simulated final function Rotator CalcDrawRotOffset()
{
	...
	local PlayerPawn PlayerOwner;
	local float UpDownSpeed;

	PlayerOwner = PlayerPawn(Owner);
	UpDownSpeed = PlayerOwner.UpDownSpeed;

	// clamp the sway -x
	// [JM] Clamp velocity, rather than pitch to smoothly come to a stop.
	if ( ( PlayerOwner.ViewRotation.Pitch == 18000 ) || ( PlayerOwner.ViewRotation.Pitch == 49152 ) )
		UpDownSpeed = 0;

	// WEAPON SWAAAAAAY -x
	// smooth out the sway with lerp
	PlayerOwner.targetpitch = lerp(10.0 * Level.TimeDeltaSeconds, PlayerOwner.targetpitch, (UpDownSpeed) * PlayerOwner.SwayIntensity / 250.0);
