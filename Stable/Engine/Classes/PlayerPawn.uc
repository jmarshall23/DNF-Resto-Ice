...
// xin :)
var globalconfig bool bStrafeRoll;
var float WeapFireGrow;
var float VibrationIntensity;
var float VibrationStart;
var float VibrationDuration;
var float LeftRightSpeed;
var float UpDownSpeed;
var globalconfig float SwayIntensity;
var globalconfig float LagIntensity;
var vector targetvel;
var float targetpitch;
var float targetyaw;
// oh you silly silly meows~
var bool AutoChargeB;
...
replication
{
	...
	unreliable if( Role==ROLE_Authority )
		SetFOVAngle, ClientShake, ClientFlash, ClientInstantFlash, AddVibration;
...
{
	/*
	VibrationTarget.X = 30 * FRand();
	VibrationTarget.Z = 30 - VibrationTarget.X;
	VibrationTime = Level.TimeSeconds;
	VibrationElasticity = 0.99;
	VibrationPeriod = 0.006;
	*/
	AddVibration(30, 1.0, 0.99, 0.006);
}

simulated function AddVibration( float Intensity, float Duration, float Elasticity, float Period )
{
	VibrationIntensity = Intensity * FRand();
	VibrationTarget.X = Intensity;
	VibrationTarget.Z = Intensity - VibrationTarget.X;
	VibrationTime = Level.TimeSeconds;
	VibrationStart = VibrationTime;
	VibrationDuration = Duration; // i cant stand this not being a thing -x
	VibrationElasticity = Elasticity;
	VibrationPeriod = Period;
}

final function float Rad(float x)
{
	return x * PI / 180;
}
...
simulated event TickVibration( float DeltaTime )
{
	...
	//if ( ((VibrationTarget.X != 0.0) || (VibrationTarget.Z != 0.0)) && (VibrationTime + VibrationPeriod < Level.TimeSeconds) )
	if (((VibrationTarget.X != 0.0) || (VibrationTarget.Z != 0.0)) && (Level.TimeSeconds < VibrationStart + VibrationDuration))
	{
		VibrationTime = Level.TimeSeconds - VibrationStart;
		scale = 1.0-(VibrationTime / VibrationDuration);

		VibrationTarget.X = Sin(Level.TimeSeconds * (VibrationElasticity * 45.f)) * (VibrationIntensity/8) * scale;
		VibrationTarget.Z = Cos(Level.TimeSeconds * (VibrationElasticity * 25.f)) * (VibrationIntensity/8) * scale;

		// i just completely rewrote it god damn -xinerki

		/*
		if ( abs(VibrationTarget.X) < 0.3 )
			VibrationTarget.X = 0.0;
		if ( abs(VibrationTarget.Z) < 0.3 )
			VibrationTarget.Z = 0.0;
		*/
	}
	VibrationVector = VibrationTarget.X*Y + VibrationTarget.Z*Z;
}
...
{
	// [JM] Moved here, as it needs to be updated for clients.
	// Shake it baby.
	if ( WeapShakeOffset > 0.0 )
	{
		WeapShakeOffset -= DeltaTime*30;
		if ( WeapShakeOffset < 0.0 )
			WeapShakeOffset = 0.0;
	}
...
event UpdateTimers( float DeltaSeconds )
{
	...
	//Auto-recharge.
	//Currently we decided against this slight auto recharge in favor of stackable energy cell items. --meows i'm overriding these dumbfucks it's now a command btfo

	if ((EnergyDrain == 0.0) && (Energy < 25) && AutoChargeB)
	Energy += DeltaSeconds;
...
function AllowRespawn()
{
	...
	EndCallbackTimer( 'ForceRespawn' );	// Make sure it's forced to be disabled
}
...
{
	bClassicShowScores = !bClassicShowScores;
}
...
// [JM] Perhaps remap sway/lag to a range of 0.0 to 1.0 (for config clarity), and do the math where appropriate?
function UpdateSway(float F)
{
	// i undid the halving of the values as lerping automatically cuts the intensity in half -x
	SwayIntensity = FClamp((F / 100.f) * 50.f, 0.0, 50.0);
}

function UpdateLag(float F)
{
	LagIntensity = FClamp((F / 100.f) * 0.001f, 0.0, 0.001);
}
...
exec function UseDown()
{
	...
	if ( (ShieldItem != None) && ShieldItem.CapturesUse() && (UseZone == 0) && (LookHitActor == None) && (Weapon.WeaponState != WS_RELOAD) )
	{
...
exec function InventoryActivate( optional float GoAwayDelay )
{
	...
	{
		// Make sure weapon actually has ammo before switching...
		if (!Weapon(SelectedItem).OutOfAmmo())
...
exec function ThrowWeapon()
{
	...
	// Don't throw your current weapon if the shield is up.
       ...
       )
	{
		...
		if ( Damage > 1 )
		{
			...
			if ( PlayerPawn( InstigatedBy ) != None )
				PlayerPawn( InstigatedBy ).ScoreHit(Self);			
...
simulated function ScoreHit(Pawn victim);
...
exec function God2()
{
	...
	{
        bTakeDamage = true;
		ClientMessage("Cheat Disabled: Super God Mode");
		return;
	}
	else
	{
        bTakeDamage = false;
		ClientMessage("Cheat Enabled: Super God Mode");
	}
}
...
exec function JoinSpectator() 
{
	if ( !PlayerReplicationInfo.bIsSpectator )
	{
		...
		//Log("[JoinSpectator] PLAYER HAS bSHADOWCAST:"$bShadowCast$" AND RECEIVE:"$bShadowReceive);
...
{	
	...
	bShadowCast 							= false;
	bShadowReceive 							= false;
	//bShadowCast 							= default.bShadowCast;
	//bShadowReceive 							= default.bShadowReceive;
	AirSpeed								= 400.0;
	Visibility								= 0;
	AttitudeToPlayer						= ATTITUDE_Friendly;
	...
	//Log("[EnterWaiting] PLAYER HAS bSHADOWCAST:"$bShadowCast$" AND RECEIVE:"$bShadowReceive);
