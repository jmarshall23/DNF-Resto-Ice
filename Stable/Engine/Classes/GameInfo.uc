...
)
{
	...
	if( StartSpot == None )
	{
        StartSpot = Spawn( class'NavigationPoint' );
	}
	...
	if( ( InSpectate != 0 ) && ( Level.NetMode == NM_DedicatedServer || Level.NetMode == NM_ListenServer ) )
	{
		...
		NewPlayer.bShadowCast 						 = false;
		NewPlayer.bShadowReceive					 = false;

		if ( NewPlayer.Shadow != None )
			NewPlayer.Shadow.Destroy();

		//Log("[Login] PLAYER HAS bSHADOWCAST:"$NewPlayer.bShadowCast$" AND RECEIVE:"$NewPlayer.bShadowReceive);
...
event AcceptInventory( pawn PlayerPawn )
{
	...
	//Log("GameInfo.uc - AcceptInventory Event Fired!");
