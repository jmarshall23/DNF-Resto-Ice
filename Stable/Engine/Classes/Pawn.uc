...
//			AddVelocity( momentum ); 
	...
	// Ch0wW - Make sure damage scaling is properly applied depending on the situation...
	if (!ClassIsChildOf( DamageType, class'FallingDamage' ))
	{
		if (InstigatedBy.bFullyShrunk && bFullyShrunk){
			Damage = Damage/(0.25);
		} else if (bFullyShrunk && !InstigatedBy.bFullyShrunk) {
			Damage *= 2;	// DOUBLE DAMAGE TO SMALL PEOPLE :>
		} else if (!bFullyShrunk && InstigatedBy.bFullyShrunk) {
			Damage /= 2;	// HALVED DAMAGE TO BIG PEOPLE :<
		}
	}
...
{
	//Log("Pawn.uc (ChangeToWeapon) - Attempting to change to: " $ NewWeapon.class.name); // [JM] Debug Shit
...
function FullyShrunken()
{
	...
	SetCallbackTimer( 10.f, false, 'RestoreShrink' );
}
...
function PartiallyShrunken()
{
	...
	SetCallbackTimer( 15.f, false, 'RestoreShrink' );
}
