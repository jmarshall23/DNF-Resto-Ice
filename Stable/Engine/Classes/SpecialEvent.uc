...
// look for a localized level message in LevelMessages.int, using Tag as key
// returns empty string if message cannot be found
function string getLevelMessage() {
	local string tagStr;
	...
	levelMessage = Localize(currentLevel, tagStr, "LevelMessages");
	errorStr = "<?int?LevelMessages."$currentLevel$"."$tagStr$"?>";
	if (levelMessage == errorStr) {
		// DEBUG
		// log("!! Missing level message for Tag \""@tagStr@"\" !!");
		return "";
	}
	
	if (levelMessage != "") return levelMessage;
...
function Trigger( actor Other, pawn EventInstigator )
{
	...
	levelMessage = getLevelMessage();
	// gracefully handle missing level message (empty string falls back to SpecialEvent.Message)
	if (levelMessage != "") {
		// TODO: consider rendering zone messages/subtitles different from console messages
		BroadcastMessage(levelMessage);
		return;
	}
