...
var				bool				bIsInventoryWeapon;			// Ch0wW - Is an inventory weapon so that weaponstay won't fuck up
...
simulated event RenderOverlays( canvas Canvas )
{
	...
	//local vector vect;
	local float FovScale, XOffset, YOffset;
	...
	SetRotation( PlayerOwner.ViewRotation + CalcDrawRotOffset() );

	FovScale = 1.0 / Tan(FClamp(PlayerOwner.DesiredFOV, 1.0, 179.0) / 360.0 * Pi);
	XOffset = (Canvas.BaseX - (Canvas.BaseX * FovScale))/2.0;
	YOffset = (Canvas.BaseY - (Canvas.BaseY * FovScale))/2.0;

	// Draw first person muzzle flash. (primary)
	// TODO: Adhere to FOV changes, issue #15.
	if ( UseSpriteFlash && (MuzzleFlashSprite != None) )
	{
		...
		Canvas.AC_SetPos((SpriteFlashX * FovScale) + XOffset, (SpriteFlashY * FovScale) + YOffset);

		if ( bMultiFrameFlash )
		{
			if (Level.TimeSeconds - MuzzleFlashTime < MuzzleFlashLength/2)
				Canvas.AC_DrawTile(MuzzleFlashSprites[0], MuzzleFlashSprite.USize * MuzzleFlashScale * FovScale, MuzzleFlashSprite.VSize * MuzzleFlashScale * FovScale, 0, 0, MuzzleFlashSprite.USize, MuzzleFlashSprite.VSize, MuzzleFlashRotation + Rand(360), , , true );
			else if	(Level.TimeSeconds - MuzzleFlashTime < MuzzleFlashLength)
				Canvas.AC_DrawTile(MuzzleFlashSprites[1], MuzzleFlashSprite.USize * MuzzleFlashScale * FovScale, MuzzleFlashSprite.VSize * MuzzleFlashScale * FovScale, 0, 0, MuzzleFlashSprite.USize, MuzzleFlashSprite.VSize, MuzzleFlashRotation + Rand(360), , , true );
		}
		else if (Level.TimeSeconds - MuzzleFlashTime < MuzzleFlashLength)
			Canvas.AC_DrawTile(MuzzleFlashSprite, MuzzleFlashSprite.USize * MuzzleFlashScale * FovScale, MuzzleFlashSprite.VSize * MuzzleFlashScale * FovScale, 0, 0, MuzzleFlashSprite.USize, MuzzleFlashSprite.VSize, MuzzleFlashRotation + Rand(360), , , true );
	...
	if ( UseAltSpriteFlash && (AltMuzzleFlashSprite != None) )
	{
		...
		Canvas.AC_SetPos((AltSpriteFlashX * FovScale) + XOffset, (AltSpriteFlashY * FovScale) + YOffset);

		if ( bAltMultiFrameFlash )
		{
			if (Level.TimeSeconds - AltMuzzleFlashTime < AltMuzzleFlashLength/2)
				Canvas.AC_DrawTile(AltMuzzleFlashSprites[0], AltMuzzleFlashSprite.USize * AltMuzzleFlashScale * FovScale, AltMuzzleFlashSprite.VSize * AltMuzzleFlashScale * FovScale, 0, 0, AltMuzzleFlashSprite.USize, AltMuzzleFlashSprite.VSize, AltMuzzleFlashRotation + Rand(360), , , true);
			else if	(Level.TimeSeconds - AltMuzzleFlashTime < AltMuzzleFlashLength)
				Canvas.AC_DrawTile(AltMuzzleFlashSprites[1], AltMuzzleFlashSprite.USize * AltMuzzleFlashScale * FovScale, AltMuzzleFlashSprite.VSize * AltMuzzleFlashScale * FovScale, 0, 0, AltMuzzleFlashSprite.USize, AltMuzzleFlashSprite.VSize, AltMuzzleFlashRotation + Rand(360), , , true);
		}
		else if (Level.TimeSeconds - AltMuzzleFlashTime < AltMuzzleFlashLength)
			Canvas.AC_DrawTile(AltMuzzleFlashSprite, AltMuzzleFlashSprite.USize * AltMuzzleFlashScale * FovScale, AltMuzzleFlashSprite.VSize * AltMuzzleFlashScale * FovScale, 0, 0, AltMuzzleFlashSprite.USize, AltMuzzleFlashSprite.VSize, AltMuzzleFlashRotation + Rand(360), , , true);
		Canvas.Style = ERenderStyle.STY_Normal;
	}
...
function bool PickupLikeWeapon( Inventory Item )
{
	...
	if ( !Weapon(item).bIsInventoryWeapon && Weapon(item).bWeaponStay && (!Weapon(item).bHeldItem || Weapon(item).bTossedOut) )
		return true;
...
function Inventory SpawnCopy( pawn Other )
{
	...
	newWeapon = Weapon(ParentCopy);
	if (!Other.bNeverSwitchOnPickup) {
		NewWeapon.ClientSelectWeapon();
		Other.ChangeToWeapon( NewWeapon );
	}
