...
var bool  MouseLeftButton, MouseRightButton;
var bool  MouseLineMode;
...
{
	if ( Action == IST_Axis && MouseCapture )
	{
		switch (Key) 
		{
			case IK_MouseX: 
...
function DrawLevelAction( canvas C )
{
	...
	else if ( Viewport.Actor.Level.LevelAction == LEVACT_LoadingNoLogo )
	{
...
event PostRender( canvas C )
{
	...
	if (MouseCapture)
	{
		if (MouseX <= 0)
			MouseX = 0;
		else if(MouseX >= C.SizeX)
			MouseX = C.SizeX - 1;
		if (MouseY <= 0)
			MouseY=0;
		else if (MouseY >= C.SizeY)
			MouseY = C.SizeY - 1;
		...
		if ( MouseBitmap == None )
			MouseBitmap = texture( DynamicLoadObject("hud_effects.ingame_hud.ingame_cursorbc", class'Texture') );

		C.Style = 3;
			
		if (MouseLineMode) 
			C.SetPos(MouseX - MouseBitmap.USize/2, MouseY - MouseBitmap.VSize / 2);
		else
			C.SetPos(MouseX, MouseY);
			
		C.DrawTile(MouseBitmap, MouseBitmap.USize, MouseBitmap.VSize, 0, 0, MouseBitmap.USize, MouseBitmap.VSize);
	}
}
...
simulated function DrawConsoleView( Canvas C )
{
	...
	for ( I = Scrollback; I < (NumLines + 1); I++ )
	{
		...
		else if ( MsgType[Line] == 'Private' )
			C.StrLen( "(Private):" $ MsgPlayer[Line].PlayerName$":"@MsgText[Line], XL, YL );			
		else
		...
		if (( MsgType[Line] == 'Say' ) || ( MsgType[Line] == 'TeamSay' ))
			...
			C.DrawText( "(Private):" $ MsgPlayer[Line].PlayerName$":"@MsgText[Line], false );
		else
