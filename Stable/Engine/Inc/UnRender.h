...
struct FSurfaceInfo
{
	...
	DWORD			PolyFlagsEx;	// Extended Surface Flags:
	...
	// JEP
	DWORD			ProjectorFlags;	// Visible projectors from this surface (each bit location represents the projector index)
};
...
//	void SetupDynamics( FSceneNode* Frame, AActor* Exclude, TArray<AActor*> &Relevent  );
	...
	// FairFriend:	Accessed externally by Direct3D8.cpp after RenDev is reinit...
	//				...to tell URender to recreate the shadow RTs. 
	UBOOL					ShadowRenderTargetsCreated; 
	INT						GMaxFinalShadowActors;
	// FairFriend: Used to keep track of the dynamic shadows resolution.
	INT						iShadowRes;
	BOOL					bShadowResChanged;
	...
	*/
	...
	// FairFriend: Used externally by Direct3D8.cpp after RenDev is reinit.
	void CreateShadowRenderTargets(UViewport* Viewport, INT NumRenderTargets);  
