...
#pragma warning(disable : 4706) /* CDH: assignment within conditional expression						*/
#pragma warning(disable : 4239) /* nonstandard extension used : 'argument'								*/
...
inline DWORD appCycles()
{
	...
	if(GTimestamp)
	{
		__asm
		{
			rdtsc	// Result returned in eax/edx
			mov [L],eax
			mov [H],edx
		}
	} 
...
inline __int64 appRDTSC()
{
#if ASM
	__asm rdtsc
#else
	return (__int64)__rdtsc();
#endif
}
