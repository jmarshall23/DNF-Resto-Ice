...
UBOOL USystem::Exec( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	else if( ParseCommand( &Cmd, TEXT("SYSDEBUG") ) )
	{
...
{
	if (!(OutMapping.FileHandle = CreateFile(Filename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL)))
		return(0);
	if (!(OutMapping.MapHandle = CreateFileMapping((HANDLE)OutMapping.FileHandle, NULL, PAGE_READONLY, 0, 0, NULL)))
	{
		CloseHandle((HANDLE)OutMapping.FileHandle);
		return(0);
	}
	if (!(OutMapping.Data = MapViewOfFile((HANDLE)OutMapping.MapHandle, FILE_MAP_READ, 0, 0, 0)))
	{
		CloseHandle((HANDLE)OutMapping.MapHandle);
		CloseHandle((HANDLE)OutMapping.FileHandle);
		return(0);
	}
	return(1);
...
CORE_API UBOOL appUnmapFile(FFileMapping& InMapping)
{
	if (!InMapping.Data)
		return(0);
	UnmapViewOfFile(InMapping.Data);
	CloseHandle((HANDLE)InMapping.MapHandle);
	CloseHandle((HANDLE)InMapping.FileHandle);
	InMapping.Data = NULL;
	return(1);
}
