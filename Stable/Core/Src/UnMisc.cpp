...
#include <wchar.h>		//TLW: for iswspace() 
#include <string.h>
...
void FOutputDevice::Log( const char* Str )
{
	if( !FName::SafeSuppressed(NAME_Log) )
		Serialize( Str, NAME_Log );
}
...
{
	volatile char DebugVersion[4096];	// Look at me when debugging :)
	if( !FName::SafeSuppressed(NAME_Log) )
	{
		TCHAR TempStr[4096];
		GET_VARARGS(TempStr,ARRAY_COUNT(TempStr),Fmt);
		strcpy((char *)DebugVersion,appToAnsi(TempStr));
...
CORE_API HINSTANCE GLaunchMutex=NULL;  // NJS: Used to track the 'single instance'
#include "FConfigCacheIni.h"

// JP_REMOVE!! (put in dnPlayerProfile.h)
extern void			dnPlayerProfile_InitPlayerProfiles();
extern UBOOL		dnPlayerProfile_ProfileExists(const TCHAR *PlayerName);
extern UBOOL		dnPlayerProfile_SelectProfile(const TCHAR *PlayerName, UBOOL WriteToHD);
extern UBOOL		dnPlayerProfile_CreateProfile(const TCHAR *PlayerName, UBOOL ForceOriginalIni);
extern FString		dnPlayerProfile_GetSavedProfile(void);
extern UBOOL		dnPlayerProfile_ResetPlayerProfile(const TCHAR *PlayerName);
...
CORE_API void appInit( const TCHAR* InPackage, const TCHAR* InCmdLine, FMalloc* InMalloc, FOutputDevice* InLog, FOutputDeviceError* InError, FFeedbackContext* InWarn, FFileManager* InFileManager, FConfigCache*(*ConfigFactory)(), UBOOL RequireConfig )
{
	...
	// (JEP) Enumerate all the player profiles
	...
	TCHAR		GProfile[256]=TEXT("");

	// Have to set GConfig before calling playerprofile code
	GConfig = ConfigFactory();

	if(!Parse( appCmdLine(), TEXT("PROFILE="), GProfile, 256 ) )
	{
		if (GIsEditor)
		{
			// If this is the editor, select the editor profile
			if (!dnPlayerProfile_ProfileExists(TEXT("DukeEd")))
				dnPlayerProfile_CreateProfile(TEXT("DukeEd"), true);		// Create editor profile if it doesn't exist yet

			CurrentProfile = TEXT("DukeEd");
		}
		else
			CurrentProfile = dnPlayerProfile_GetSavedProfile();				// Select the last known profile
	}
	else
		CurrentProfile = GProfile;		// Set the profile to what the cmd line says

	// If the current profile does not exist, create the DukeTemp profile, and use that
	if (CurrentProfile == TEXT("") || !dnPlayerProfile_ProfileExists(*CurrentProfile))
	{
		if (!dnPlayerProfile_ProfileExists(TEMP_PROFILE))
			dnPlayerProfile_CreateProfile(TEMP_PROFILE, true);		// Create it for the first time
		else
			dnPlayerProfile_ResetPlayerProfile(TEMP_PROFILE);		// Restore it (so it's ALWAYS fresh)

		CurrentProfile = TEMP_PROFILE;
	}

	debugf( NAME_Init, TEXT("Profile: %s"), *CurrentProfile);

	// Select the current profile
	if (!dnPlayerProfile_SelectProfile(*CurrentProfile, false))
		appErrorf( TEXT("dnPlayerProfile_SelectProfile failed"));
#else		// ... JEP
	// Ini.
	TCHAR GIni[256]=TEXT("");
...
#if DNF
	GDnExec->LoadConfigVariables(); // CDH
#endif
...
#if DNF
	GDnExec->SaveConfigVariables(); // CDH
#endif
	appPlatformExit();
	if( GConfig )
	{
...
/*-----------------------------------------------------------------------------
	Fast Squareroot.
-----------------------------------------------------------------------------*/
#if 0	// NJS: Fast squareroot table
CORE_API unsigned int fast_sqrt_table[0x10000];  // declare table of square roots 
CORE_API bool sqrtInitialized;
#endif
