...
static void PentiumPolyC32Modulated()
{
	...
	do
	{
		...
		DWORD R = Min<FLOAT>(appRound ((FLOAT)( (*Dest ) & 0xff0000 ) * FloatPalBase[I*4+0]* (1.0/128.0)),255<<16);
		DWORD G = Min<FLOAT>(appRound ((FLOAT)( (*Dest ) & 0x00ff00 ) * FloatPalBase[I*4+1]* (1.0/128.0)),255<< 8);
		DWORD B = Min<FLOAT>(appRound ((FLOAT)( (*Dest ) & 0x0000ff ) * FloatPalBase[I*4+2]* (1.0/128.0)),255<< 0);
...
inline void USoftwareRenderDevice::InnerGouraudMMX32(DWORD PolyFlags, INT MinY, INT MaxY, FSceneNode* Frame,FMipmapBase* Mip,FSpanBuffer* SpanBuffer)
{
...
inline void USoftwareRenderDevice::InnerGouraudMMX15(DWORD PolyFlags, INT MinY, INT MaxY, FSceneNode* Frame,FMipmapBase* Mip,FSpanBuffer* SpanBuffer)
{
	static  BYTE*	TexBase;
	static  FMMX*	PalBase;
	static  FMMX	MMXCoMask,MMXCoShift; 
...
inline void USoftwareRenderDevice::InnerGouraudMMX16(DWORD PolyFlags, INT MinY, INT MaxY, FSceneNode* Frame,FMipmapBase* Mip,FSpanBuffer* SpanBuffer)
{
	static  BYTE*	TexBase;
	static  FMMX*	PalBase;
	static  FMMX	MMXCoMask,MMXCoShift; 
...
)
{
	...
	{  
		debugf(NAME_Log,TEXT("Indefinite ScreenY triangle coordinate error."));
		return;
	}
	...
	*(FLOAT*)&ExpV = (FLOAT)dVdX;
	...
	if (PolyFlags & PF_RenderFog)		
	{
		...
		TriVertex[0].Fog.X = ( Point0->Fog.X * GFloatScale.X + GFloatFog.X ); 
		TriVertex[0].Fog.Y = ( Point0->Fog.Y * GFloatScale.Y + GFloatFog.Y ); 
		TriVertex[0].Fog.Z = ( Point0->Fog.Z * GFloatScale.Z + GFloatFog.Z ); 

		TriVertex[1].Fog.X = ( Point1->Fog.X * GFloatScale.X + GFloatFog.X ); 
		TriVertex[1].Fog.Y = ( Point1->Fog.Y * GFloatScale.Y + GFloatFog.Y ); 
		TriVertex[1].Fog.Z = ( Point1->Fog.Z * GFloatScale.Z + GFloatFog.Z ); 

		TriVertex[2].Fog.X = ( Point2->Fog.X * GFloatScale.X + GFloatFog.X ); 
		TriVertex[2].Fog.Y = ( Point2->Fog.Y * GFloatScale.Y + GFloatFog.Y ); 
		TriVertex[2].Fog.Z = ( Point2->Fog.Z * GFloatScale.Z + GFloatFog.Z ); 
		...
		TriVertex[0].Light.X =Point0->Light.X * (1.f-Point0->Fog.X) * GFloatScale.X;
		TriVertex[0].Light.Y =Point0->Light.Y * (1.f-Point0->Fog.Y) * GFloatScale.Y;
		TriVertex[0].Light.Z =Point0->Light.Z * (1.f-Point0->Fog.Z) * GFloatScale.Z;

		//FLOAT FogScale1 = 1.0f-Point1->Fog.A;

		TriVertex[1].Light.X =Point1->Light.X * (1.f-Point1->Fog.X) * GFloatScale.X;
		TriVertex[1].Light.Y =Point1->Light.Y * (1.f-Point1->Fog.Y) * GFloatScale.Y;
		TriVertex[1].Light.Z =Point1->Light.Z * (1.f-Point1->Fog.Z) * GFloatScale.Z;

		//FLOAT FogScale2 = 1.0f-Point2->Fog.A;

		TriVertex[2].Light.X =Point2->Light.X * (1.f-Point2->Fog.X) * GFloatScale.X;
		TriVertex[2].Light.Y =Point2->Light.Y * (1.f-Point2->Fog.Y) * GFloatScale.Y;
		TriVertex[2].Light.Z =Point2->Light.Z * (1.f-Point2->Fog.Z) * GFloatScale.Z;

	}
	else // Use the uniform screenflash-fog instead:
	{
		TriVertex[0].Fog.X = GFloatFog.X;
		TriVertex[0].Fog.Y = GFloatFog.Y; 
		TriVertex[0].Fog.Z = GFloatFog.Z; 

		TriVertex[1].Fog.X = GFloatFog.X; 
		TriVertex[1].Fog.Y = GFloatFog.Y; 
		TriVertex[1].Fog.Z = GFloatFog.Z; 

		TriVertex[2].Fog.X = GFloatFog.X; 
		TriVertex[2].Fog.Y = GFloatFog.Y; 
		TriVertex[2].Fog.Z = GFloatFog.Z; 

		TriVertex[0].Light.X = MinPositiveFloat( Point0->Light.X * GFloatScale.X , GFloatRange.X );
		TriVertex[0].Light.Y = MinPositiveFloat( Point0->Light.Y * GFloatScale.Y , GFloatRange.Y );
		TriVertex[0].Light.Z = MinPositiveFloat( Point0->Light.Z * GFloatScale.Z , GFloatRange.Z );
			
		TriVertex[1].Light.X = MinPositiveFloat( Point1->Light.X * GFloatScale.X , GFloatRange.X );
		TriVertex[1].Light.Y = MinPositiveFloat( Point1->Light.Y * GFloatScale.Y , GFloatRange.Y );
		TriVertex[1].Light.Z = MinPositiveFloat( Point1->Light.Z * GFloatScale.Z , GFloatRange.Z );

		TriVertex[2].Light.X = MinPositiveFloat( Point2->Light.X * GFloatScale.X , GFloatRange.X );
		TriVertex[2].Light.Y = MinPositiveFloat( Point2->Light.Y * GFloatScale.Y , GFloatRange.Y );
		TriVertex[2].Light.Z = MinPositiveFloat( Point2->Light.Z * GFloatScale.Z , GFloatRange.Z );
	}

	    
	FLOAT  dRdX  = (TriVertex[0].Light.X - TriVertex[2].Light.X)*SD12y - (TriVertex[1].Light.X - TriVertex[2].Light.X)*SD02y;
	FLOAT  dGdX  = (TriVertex[0].Light.Y - TriVertex[2].Light.Y)*SD12y - (TriVertex[1].Light.Y - TriVertex[2].Light.Y)*SD02y;
	FLOAT  dBdX  = (TriVertex[0].Light.Z - TriVertex[2].Light.Z)*SD12y - (TriVertex[1].Light.Z - TriVertex[2].Light.Z)*SD02y;
	MMX15Load(TriDeltaLight,dRdX,dGdX,dBdX);

	FLOAT  dRFdX = (TriVertex[0].Fog.X - TriVertex[2].Fog.X)*SD12y - (TriVertex[1].Fog.X - TriVertex[2].Fog.X)*SD02y;
	FLOAT  dGFdX = (TriVertex[0].Fog.Y - TriVertex[2].Fog.Y)*SD12y - (TriVertex[1].Fog.Y - TriVertex[2].Fog.Y)*SD02y;
	FLOAT  dBFdX = (TriVertex[0].Fog.Z - TriVertex[2].Fog.Z)*SD12y - (TriVertex[1].Fog.Z - TriVertex[2].Fog.Z)*SD02y;
...
guardSlow(FlashTriangle EdgeSetup);
	...
	do {	
		...
		if ( TB->YFloor < TA->YFloor )
		{
			...
			FLOAT DR      = RDY * ( TA->Light.X - TB->Light.X );
			FLOAT DG      = RDY * ( TA->Light.Y - TB->Light.Y );
			FLOAT DB      = RDY * ( TA->Light.Z - TB->Light.Z );
			MMX15Load(EdgeSetupPtr->DeltaLightRGB, DR, DG, DB);

			FLOAT R       = TB->Light.X + YAdj * DR;
			FLOAT G       = TB->Light.Y + YAdj * DG;
			FLOAT B       = TB->Light.Z + YAdj * DB;
			MMX15Load(EdgeSetupPtr->LightRGB, R, G, B);

			FLOAT DFR     = RDY * (TA->Fog.X - TB->Fog.X);
			FLOAT DFB     = RDY * (TA->Fog.Y - TB->Fog.Y);
			FLOAT DFG     = RDY * (TA->Fog.Z - TB->Fog.Z);
			MMX14Load(EdgeSetupPtr->DeltaFogRGB,DFR,DFG,DFB);

			FLOAT FR	   = TB->Fog.X + YAdj * DFR;
			FLOAT FG	   = TB->Fog.Y + YAdj * DFG;
			FLOAT FB	   = TB->Fog.Z + YAdj * DFB;
...
*/
	...
	do {	
		if( TB->YFloor != TA->YFloor )
		{
			if ( TB->YFloor < TA->YFloor )
			{
				...
				FLOAT DR      = RDY * (TA->Light.X - TB->Light.X);
				FLOAT DB      = RDY * (TA->Light.Y - TB->Light.Y);
				FLOAT DG      = RDY * (TA->Light.Z - TB->Light.Z);

				FLOAT R       = TB->Light.X + YAdj * DR; // prestepping required here to prevent overflow.
				FLOAT G       = TB->Light.Y + YAdj * DG; 
				FLOAT B       = TB->Light.Z + YAdj * DB;
	...
	*(INT*)&DV = appRound(dVdX);
	...
	if( (Viewport->ColorBytes==2) && (Viewport->Caps & CC_RGB565) )
	{
		...
		{
			GPolyC.FlashR	= (3<<22) + (GFloatFog.X* 32.0f)*(1<<11); // >> 3); // Built-in alignment forcing factor + fog offset.
			GPolyC.FlashG	= (3<<22) + (GFloatFog.Y* 64.0f)*(1<< 5); // >> 2); //
			GPolyC.FlashB	= (3<<22) + (GFloatFog.Z* 32.0f);		  // >> 3); //
		}
	}
	else if ( Viewport->ColorBytes==2 )
	{
		...
		{
			GPolyC.FlashR	= (FLOAT)(3<<22) + (GFloatFog.X* 32.0f)*(1<<10); // >> 3);  // Built-in alignment forcing factor + fog offset.
			GPolyC.FlashG	= (FLOAT)(3<<22) + (GFloatFog.Y* 32.0f)*(1<<5);  // >> 3);  // 
			GPolyC.FlashB	= (FLOAT)(3<<22) + (GFloatFog.Z* 32.0f);         // >> 3);  //
		}
	}
	else if( Viewport->ColorBytes==4 )
	{
		...
		{
			GPolyC.FlashR	= (FLOAT)(3<<22) + GFloatFog.X * 256.0f;  // Built-in alignment forcing factor + fog offset.
			GPolyC.FlashG	= (FLOAT)(3<<22) + GFloatFog.Y * 256.0f;  //
			GPolyC.FlashB	= (FLOAT)(3<<22) + GFloatFog.Z * 256.0f;  //
...
//	guard(USoftwareRenderDevice::DrawGouraudPolygon);
	...
	*/
...
#if DO_GUARD_SLOW
	{
		...
		if (MinY < 0) appErrorf(_T(" SpanStartY negative!"));
		if (MaxY > Viewport->SizeY) appErrorf(_T(" SpanEndY bigger than ViewportSizeY!"));
		...
		for( INT Y=MinY; Y<MaxY; Y++ )
		{
			for( FSpan* Span=*Index++; Span; Span=Span->Next )
			{
				...
				if ( (X0 <0) || ( X1 > Viewport->SizeX))
					appErrorf( _T("Span out of bounds. Ypos: %i XStart: %i XEnd: %i"),Y, Span->Start,Span->End);
			}
		}
	}
#endif
