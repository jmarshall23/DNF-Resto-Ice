...
simulated function EvalBones()
{
	...
	local vector t, StretchWorldOffset;
	local Actor LocalClimber;

	// This gets spammed heavily every now and then:
	// ScriptWarning: AIClimbControl !z1l1_4.AIClimbControl___1 (Function dnAI.AIClimbControl.EvalBones:000C) Accessed None
	...
	if (LocalClimber == None)
		return;

	bone = minst.BoneFindNamed('End');
	if (bone == 0)
		return;
	endMinst = LocalClimber.GetMeshInstance();
	if (endMinst != None)
		StretchWorldOffset += endMinst.MeshToWorldLocation(vect(13,0,0)) - LocalClimber.Location;
	t = minst.WorldToMeshLocation(LocalClimber.Location + StretchWorldOffset);
