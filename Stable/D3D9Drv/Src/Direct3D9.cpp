/*=============================================================================
	Direct3D9.cpp: Duke Forever Direct3D 9 support.
	...
	Revision history:
		...
		* Upgraded to Direct3D 9 by Raziel
=============================================================================*/
...
#define ENABLE_RAZIEL_VERTEX_EXPLOSION_FIX
...
#define PARTICLE_VERTEXBUFFER_SIZE	    32768  // NJS: Expand when ready.
...
#define D3D_CHECK(VALUE) \
	...
	{\
		...
		if(FAILED(__hr))\
		{		\
			appErrorf(TEXT("[%s, %i] D3D ERROR: ") TEXT(#VALUE) TEXT(" == %s"),appFromAnsi(__FILE__),__LINE__,DXGetErrorString(__hr));\
		}\
	} while(0)
...
//#define DISABLE_PROJECTOR_POLYS
#define PROJECTOR_POLYS_Z_BIAS 10.0f
...
public:
	...
	IDirect3DVertexBuffer9*	VertexBuffer9;
	INT						Length,
							First,
							Rover;
	...
	void Init(IDirect3DDevice9*	InDevice, INT InLength)
	{
	...
	T* Lock(INT Num, INT Start=-1)
	{
		...
		if(Start!=-1)
		{
			D3D_CHECK(VertexBuffer9->Lock(Start * sizeof(T),Num * sizeof(T),(VOID**) &VertexData,D3DLOCK_NOOVERWRITE));
		} 
		else if(Rover + Num < Length)
		{
			D3D_CHECK(VertexBuffer9->Lock(Rover * sizeof(T),Num * sizeof(T),(VOID**) &VertexData,D3DLOCK_NOOVERWRITE));
			First =Rover;
		...
		{
			D3D_CHECK(VertexBuffer9->Lock(0,Num * sizeof(T),(VOID**) &VertexData,D3DLOCK_DISCARD));
			First=0;
			Rover=Num;
		}
	...
	INT Unlock()
	{
		...
		D3D_CHECK(VertexBuffer9->Unlock());
	...
	void Set()
	{
		...
		D3D_CHECK(Device->SetFVF(T::FVF));
		D3D_CHECK(Device->SetStreamSource(0,VertexBuffer9,0,sizeof(T)));
	}
};
...
class DLL_EXPORT UD3DRenderDevice : public URenderDevice
{
	...
	struct ProjectorInfo
	{
		...
		IDirect3DSurface9		*pRenderTargetSurf;
	...
	IDirect3DSurface9		*pOriginalRenderTarget;		// Used to restore back to original render target
	IDirect3DSurface9		*pOriginalZStencil;			// Used to restore back to original zstencil
	IDirect3DTexture9		*ClipperTexture;			// Texture used to clip textures to the projector frustum
	void					*TempRT;
	...
	static void __fastcall InitD3D()
	{
		...
		verify(Direct3D9=Direct3DCreate9(D3D_SDK_VERSION));
		...
		for(INT Index=0;Index<NumAdapters;Index++)
		{
			...
			D3D_CHECK(Direct3D9->GetAdapterIdentifier(Index, 0, &AdapterIdentifier));
		...
		D3D_CHECK(Direct3D9->GetDeviceCaps(BestAdapterIndex,D3DDEVTYPE_HAL,&DeviceCaps9));
		...
		D3D_CHECK(Direct3D9->GetAdapterDisplayMode(BestAdapterIndex,&OriginalDisplayMode));
	}
	...
	void __fastcall Exit()
	{
		...
		else
		{
			...
			SAFETRY(SafeRelease(Direct3DDevice9));
		}
	}
	...
	void UpdateGamma( UViewport* Viewport )
	{
		...
		if( Direct3DDevice9 && (ViewportFullscreen||UseWindowedGamma) && (DeviceCaps9.Caps2 & D3DCAPS2_FULLSCREENGAMMA) )
		{
	...
	void __fastcall Flush( UBOOL AllowPrecache )
	{
		if( Direct3DDevice9 )
		{
			for( DWORD i=0; i<DeviceCaps9.MaxSimultaneousTextures; i++ )
			{
				SetTextureNULL(i);
			}
			...
			while(CachedTextures)
			{
				...
				if(TexInfo->Texture9)
					D3D_CHECK(SafeRelease(TexInfo->Texture9));
			...
			if( (ViewportFullscreen||UseWindowedGamma) && DeviceCaps9.Caps2 & D3DCAPS2_FULLSCREENGAMMA)
			{
				FLOAT Brightness = Viewport->GetOuterUClient()->Brightness;
			...
			Direct3DDevice9->SetStreamSource(0,NULL,0,0);
			Direct3DDevice9->SetIndices(NULL);
		}
	...
	void __fastcall PreRender( FSceneNode* Frame )
	{
		...
		Direct3DDevice9->SetTransform( D3DTS_VIEW, &ViewMatrix );
		...
		{
			// Compute Hor+ FOV.
			FLOAT Ratio = (Frame->Viewport->SizeX / (FLOAT)Frame->Viewport->SizeY) / (4 / 3.f);
			FLOAT FOV   = (180.f / PI) * 2 * appAtan(appTan((Frame->Viewport->Actor->FovAngle * PI / 180.f) / 2) * Ratio);
		...
		Direct3DDevice9->SetRenderState( D3DRS_LIGHTING, 0 ); 
	}
	...
	void __fastcall Lock( FColor FogColor, float FogDensity, INT FogDistance, FPlane InFlashScale, FPlane InFlashFog, FPlane ScreenClear, DWORD InLockFlags, BYTE* InHitData, INT* InHitSize )
	{
		...
		verify(Direct3DDevice9);
		hr=Direct3DDevice9->TestCooperativeLevel();
		if( hr!=D3D_OK )
		{
			debugf(TEXT("TestCooperativeLevel failed (%s)"),DXGetErrorString(hr));
			Failed:
			...
			do hr2=Direct3DDevice9->TestCooperativeLevel();
			while(hr2==D3DERR_DEVICELOST);

			if(hr2==D3DERR_DEVICENOTRESET)
			{
				debugf(TEXT("Resetting mode (%s)"),DXGetErrorString(hr2));
				if( !SetRes(ViewportX, ViewportY, ViewportColorBits/8, ViewportFullscreen) )
					if(!ErrorCalled)
					{
						ErrorCalled=true;
						appErrorf(TEXT("Failed resetting mode. (%s)"),DXGetErrorString(hr2));
					}
			}
		}
		...
		// Clear the Z-buffer.
		Direct3DDevice9->Clear( 0, NULL, D3DCLEAR_ZBUFFER | ((LockFlags & LOCKR_ClearScreen) ? D3DCLEAR_TARGET : 0), (D3DCOLOR)FColor(ScreenClear).TrueColor(), 1.f, 0 );
		...
		if( FAILED(h=Direct3DDevice9->BeginScene()) )
		{
			if( ++FailCount==1 )
				goto Failed;

			appErrorf(TEXT("BeginScene failed (%s)"),DXGetErrorString(h));
		}
	...
	void __fastcall DrawComplexSurface( FSceneNode* Frame, FSurfaceInfo& Surface, FSurfaceFacet& Facet )
	{
		...
		if( /*UseMultitexture &&*/ Surface.LightMap!=NULL && Surface.MacroTexture==NULL )
		{
			...
			// Raziel note: Actual world (brush) geometry rendering happens here, not static/skeletal meshes
			for( Poly=Facet.Polys; Poly; Poly=Poly->Next)
			{
				Direct3DDevice9->DrawPrimitive( D3DPT_TRIANGLEFAN, First, Poly->NumPts - 2 );
				First += Poly->NumPts;
				Stats.Polys++;
			}
		...
		{			
			...
			{				
				...
				for( FSavedPoly* Poly=Facet.Polys; Poly; Poly=Poly->Next )
				{
					...
					if( CountNear )
					{
						...
						{
							SetBlending( PF_Modulated );
							Direct3DDevice9->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_BLENDDIFFUSEALPHA );
							SetZBias(15.f);
						...
						Direct3DDevice9->DrawPrimitive( D3DPT_TRIANGLEFAN, First, NumNear - 2 );
						Stats.Polys++;
					}							
					n += Poly->NumPts;
				}
			...
			if( AreDetailing )
			{
				Direct3DDevice9->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_MODULATE );
				SetZBias(0);
				AreDetailing = 0;
			}
		}
		...
		if( Surface.PolyFlags & PF_FlatShaded )  // Wireframe Overlay
		{
			...
			//First=OriginalFirst;
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_DISABLE  );
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_ALPHAOP, D3DTOP_DISABLE );
			
			for( Poly=Facet.Polys; Poly; Poly=Poly->Next)
			{
				Direct3DDevice9->DrawPrimitive( D3DPT_LINESTRIP,First,Poly->NumPts-1); 
				First += Poly->NumPts;
			}
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_MODULATE );
		...
		#ifndef DISABLE_PROJECTOR_POLYS
		if (ProjectorArray.Num() > 0 && Frame->Recursion == 0 && Surface.ProjectorFlags)
		{
			...
			Direct3DDevice9->SetTexture(1, ClipperTexture);
			Direct3DDevice9->SetTextureStageState( 1, D3DTSS_COLORARG1, D3DTA_TEXTURE );
			Direct3DDevice9->SetTextureStageState( 1, D3DTSS_COLOROP, D3DTOP_ADD);
			//Direct3DDevice9->SetTextureStageState( 1, D3DTSS_COLOROP, D3DTOP_DISABLE);
			Direct3DDevice9->SetTextureStageState( 1, D3DTSS_ALPHAOP, D3DTOP_DISABLE);

			Direct3DDevice9->SetSamplerState( 1, D3DSAMP_ADDRESSU, D3DTADDRESS_CLAMP );
			Direct3DDevice9->SetSamplerState( 1, D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP );
			
			Direct3DDevice9->SetSamplerState( 1, D3DSAMP_MAGFILTER, D3DTEXF_POINT);
			Direct3DDevice9->SetSamplerState( 1, D3DSAMP_MINFILTER, D3DTEXF_POINT);
			...
			for (int i=ProjectorArray.Num()-1; i>=0 ; i--)
			{
				...
				Direct3DDevice9->SetTexture(0, pProjector->pRenderTargetTex);
				...
				for( Poly=Facet.Polys; Poly; Poly=Poly->Next)
				{
					if (!(pProjector->GNodes[Poly->iNode].ProjectorFlags & Mask))
						continue;
					// Raziel note: Shadow rendering for brush geometry in non-batched mode
					SetZBias(PROJECTOR_POLYS_Z_BIAS); // Raziel: Add a Z bias to fix shadows
					Direct3DDevice9->DrawPrimitive( D3DPT_TRIANGLEFAN, First, Poly->NumPts - 2 );
					SetZBias();
					First += Poly->NumPts;
				}
			...
			Direct3DDevice9->SetSamplerState( 1, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR );
			Direct3DDevice9->SetSamplerState( 1, D3DSAMP_MINFILTER, D3DTEXF_LINEAR );

			Direct3DDevice9->SetSamplerState( 1, D3DSAMP_ADDRESSU, D3DTADDRESS_WRAP );
			Direct3DDevice9->SetSamplerState( 1, D3DSAMP_ADDRESSV, D3DTADDRESS_WRAP );
	...
	void __fastcall QueuePolygonEnd(DWORD ProjectorFlags = 0)
	{
		...
		for(i=0;i<QueuedPolygonCount;)
		{
			...
			{
				CurrentDoFog=true;
				Direct3DDevice9->SetTextureStageState( 1, D3DTSS_COLORARG1, D3DTA_SPECULAR );			
				Direct3DDevice9->SetTextureStageState( 1, D3DTSS_COLOROP,   D3DTOP_ADD );
			...
			// Raziel note: Skeletal mesh rendering happens here
			int count=j-i;

			if (p.PolyFlagsEx&PFX_Translucent2)
			{
				// Raziel: If this is a mesh with translucency, do a depth pass before rendering the actual mesh
				DrawPrimitiveWithDepthPass(D3DPT_TRIANGLELIST, First + (i * 3), count);
			} 
			else
			{
				Direct3DDevice9->DrawPrimitive(D3DPT_TRIANGLELIST, First + (i * 3), count);
			}
	...
	void __fastcall DrawPrimitiveWithDepthPass(D3DPRIMITIVETYPE PrimitiveType, UINT StartVertex, UINT PrimitiveCount)
	{
		// Raziel note: This is a new function added to render meshes with a depth pass
		
		// Set up for rendering a depth only pass
		BOOL OriginalAlphaBlendEnable = SetAlphaBlendEnable(TRUE);
		D3DBLEND OriginalSourceBlend = SetSrcBlend(D3DBLEND_ZERO);
		D3DBLEND OriginalDestinationBlend = SetDstBlend(D3DBLEND_ONE);
		BOOL OriginalZWriteEnable = SetZWriteEnable(TRUE);
		D3DCMPFUNC OriginalZFunc = SetZFunc(D3DCMP_LESSEQUAL);

		// Do a depth only render pass
		Direct3DDevice9->DrawPrimitive(PrimitiveType, StartVertex, PrimitiveCount);

		// Restore most of the render state to render the actual mesh
		SetAlphaBlendEnable(OriginalAlphaBlendEnable);
		SetSrcBlend(OriginalSourceBlend);
		SetDstBlend(OriginalDestinationBlend);
		SetZWriteEnable(OriginalZWriteEnable);

		// Render the mesh, taking depth into consideration
		Direct3DDevice9->DrawPrimitive(PrimitiveType, StartVertex, PrimitiveCount);

		// Finally, we can restore the original Z function as well
		SetZFunc(OriginalZFunc);
	}

	float __fastcall GetDynamicZPlaneDistance(FTransTexture** Pts)
	{
		// Raziel: Part of the vertex explosion fix. Weapons need to use a closer Z plane to objects in the world,
		//  otherwise those objects can end up warping due to being clipped off too close to the camera. On the
		//  other hand, the weapon geometry would get clipped off if we only used a clipping plane with a distance
		//  appropriate for the world geometry. So I switch between two Z planes depending on the distance.
		// 
		//  NOTE: If you alter these settings, be sure to test that the game does not crash without the bias in the
		//  QueuePolygon() function. After testing, be sure to re-enable the bias.
		for (int i = 0; i < 3; i++)
		{
			if (Pts[i]->Point.Z > 10.0f)
				return 5.0f;
		}
		return 1.0f;
	}

	FTransTexture __fastcall ClipVertexToZPlane(FTransTexture* Point1, FTransTexture* Point2, float ZPlaneDistance)
	{
		// Raziel: Part of the vertex explosion fix. Given two vertices (Point1 in front of the clipping plane, and
		//  Point2 behind the clipping plane), this function will return a reprojected point on the Z plane based
		//  on the distance to the Z plane (which is passed to the function). If you want to understand what this
		//  function is doing, you can refer to the 'Clipping Calculation' section on this page:
		//  http://tfpsly.free.fr/Docs/TomHammersley/frustum.htm
		FTransTexture outVertex;

		// Calculate t, the point between the two vertices where the edge intersects the plane, in the range 0 to 1
		float pointDistanceZ = Point2->Point.Z - Point1->Point.Z;	// Z distance between point 1 and 2
		float planeDistanceZ = ZPlaneDistance - Point1->Point.Z;	// Z distance between point 1 and the clipping plane
		FTransTexture direction = *Point2 - *Point1;				// The direction vector from Point1 to Point2
		float t = planeDistanceZ / pointDistanceZ;					// Point at which the direction vector is sliced by the clipping plane
		
		
		// Create the output vertex = <Point1> + (<Direction from Point1 to Point2> * <t, the percentage of the direction vector to the clipping plane>)
		outVertex = *Point1 + (direction * t); // This handles most of the internal state automatically, but not the screen coordinate projection
		outVertex.Flags = Point2->Flags; // Since we are replacing Point2, we should use the flags from that point
		outVertex.Project(CurrentFrame); // Reproject into screen coordinates

		return outVertex;
	}

	void __fastcall ClipAndQueuePolygon(FTextureInfo* Info, FTransTexture** Pts, DWORD PolyFlags, DWORD ExFlags, FSpanBuffer* Span, float ZPlaneDistance)
	{
		// Raziel: Part of the vertex explosion fix. This function takes a single triangle and slices it along the Z plane based on
		//  the distance passed to the function. Depending on the scenario, either one or two triangles may be created in place of
		//  the original. If all the vertices are behind the clipping plane, it will not queue the triangle for rendering. If you
		//  want to understand what this function is doing, you can refer to the 'Clipping Triangles' section on this page:
		//  https://gabrielgambetta.com/computer-graphics-from-scratch/11-clipping.html
		FTransTexture** triangle = new FTransTexture * [3];

		// These variables store the index of the last vertex in front and behind the clipping plane which we need later on
		int vertextBehindPlaneIndex;
		int vertextInFrontPlaneIndex;

		// The code below works out how many vertices are behind the clipping plane, which feeds into how we should handle the
		//  triangle and records the index of the last vertex in front of and behind the clipping plane
		int numVerticesBehindPlane = 0;
		for (int i = 0; i < 3; i++)
		{
			if (Pts[i]->Point.Z < ZPlaneDistance)
			{
				// Vertex is behind the clipping plane
				numVerticesBehindPlane++;
				vertextBehindPlaneIndex = i;
			}
			else
			{
				vertextInFrontPlaneIndex = i;
			}
		}

		// First scenario, all 3 vertices are behind the clip plane, so the triangle doesn't need to be rendered
		if (numVerticesBehindPlane == 3)
			return;

		// Second scenario, two vertices are behind the clip plane, this will produce one triangle for rendering
		if (numVerticesBehindPlane == 2)
		{
			int triIndex = vertextInFrontPlaneIndex;
			triangle[0] = Pts[triIndex];

			triIndex = ++triIndex > 2 ? 0 : triIndex; // Get next vertex index
			FTransTexture clippedVertex1 = ClipVertexToZPlane(Pts[vertextInFrontPlaneIndex], Pts[triIndex], ZPlaneDistance);
			triangle[1] = &clippedVertex1;

			triIndex = ++triIndex > 2 ? 0 : triIndex; // Get next vertex index
			FTransTexture clippedVertex2 = ClipVertexToZPlane(Pts[vertextInFrontPlaneIndex], Pts[triIndex], ZPlaneDistance);
			triangle[2] = &clippedVertex2;

			QueuePolygon(Info, triangle, 3, PolyFlags, ExFlags, Span);

			return;
		}

		// Third scenario, one vertex is behind the clip plane, this will produce two triangles for rendering
		if (numVerticesBehindPlane == 1)
		{
			int triIndex3 = vertextBehindPlaneIndex;			// Get the third vertex index (the one behind the plane)
			int triIndex1 = triIndex3 > 1 ? 0 : triIndex3 + 1;	// Get first vertex index (the one after the vertex behind the plane)
			int triIndex2 = triIndex1 > 1 ? 0 : triIndex1 + 1;	// Get second vertex index (the one after the first vertex index)

			triangle[0] = Pts[triIndex1];
			triangle[1] = Pts[triIndex2];
			FTransTexture clippedVertex1 = ClipVertexToZPlane(Pts[triIndex1], Pts[triIndex3], ZPlaneDistance);
			triangle[2] = &clippedVertex1;

			QueuePolygon(Info, triangle, 3, PolyFlags, ExFlags, Span);

			triangle[0] = &clippedVertex1;
			triangle[1] = Pts[triIndex2];
			FTransTexture clippedVertex2 = ClipVertexToZPlane(Pts[triIndex2], Pts[triIndex3], ZPlaneDistance);
			triangle[2] = &clippedVertex2;
...
#ifdef ENABLE_RAZIEL_VERTEX_EXPLOSION_FIX
		// Raziel: Part of the vertex explosion fix. This code checks if any of the vertices in the triangle clips into the
		//  Z plane, if so, it will use my clipping code to cut the geometry at the clipping plane, ensuring geometry does
		//  not get close enough to the camera to 'explode'.
		float ZPlaneDistance = GetDynamicZPlaneDistance(Pts);
		float ZPlaneDistanceWithBias = ZPlaneDistance - 0.001f;
		for (int i = 0; i < 3; i++)
		{
			// If this triangle has vertices behind the clip plane, we need to clip the triangle and queue the clipped version
			if (Pts[i]->Point.Z < ZPlaneDistanceWithBias)
			{
				// This triangle requires clipping before it can be queued
				ClipAndQueuePolygon(Info, Pts, PolyFlags, ExFlags, Span, ZPlaneDistance);

				// The triangle will already be queued by the call above, so we can return from the function at this point
				return;
			}
		}
#endif//ENABLE_RAZIEL_VERTEX_EXPLOSION_FIX
	...
	void __fastcall QueuePolygonEndFast()
	{
		...
		Direct3DDevice9->DrawPrimitive( D3DPT_TRIANGLELIST, First, QueuedPolygonCount);
	...
	void RenderQueuedPolygonsForProjectors(DWORD ProjectorFlags)
	{
		...
		for (int p=ProjectorArray.Num()-1; p>=0 ; p--)
		{
			...
			if (NumMaskedPolys)
			{
				for(i=0;i<QueuedPolygonCount;)
				{
					...
					// Raziel note: Shadow rendering for mesh geometry in non-batched mode
					SetZBias(PROJECTOR_POLYS_Z_BIAS); // Raziel: Add a Z bias to fix shadows
					Direct3DDevice9->DrawPrimitive( D3DPT_TRIANGLELIST, First+(i*3), Count);
					SetZBias();
		...
		Direct3DDevice9->SetTexture(0, NULL);

		SetAlphaBlendEnable(TRUE);
		SetSrcBlend(D3DBLEND_DESTCOLOR);
		SetDstBlend(D3DBLEND_SRCCOLOR);

		Direct3DDevice9->SetTexture(1, NULL);
		Direct3DDevice9->SetTextureStageState( 1, D3DTSS_COLOROP, D3DTOP_DISABLE);
	...
	void __fastcall DrawGouraudPolygon( FSceneNode* Frame, FTextureInfo& Info, FTransTexture** Pts, INT NumPts, DWORD PolyFlags, FSpanBuffer* Span, DWORD PolyFlagsEx )
	{
...
#ifdef ENABLE_RAZIEL_VERTEX_EXPLOSION_FIX
		// Raziel: Part of the vertex explosion fix. Decals like blood stains are rendered using this function which can explode
		//  for similar reasons to meshes. Since these decals are usually quite small, we can get away with simply not rendering
		//  them if they get close to or behind the camera's Z plane as it should be out of view by that point anyway.
		for (int i = 0; i < NumPts; i++)
		{
			if (Pts[i]->Point.Z < 0.0001f)
			{
				// This triangle has vertices very close to or behind the camera's Z point, so skip its rendering
				return;
			}
		}
#endif//ENABLE_RAZIEL_VERTEX_EXPLOSION_FIX
		...
		if ( DoFog )
		{
			...
			Direct3DDevice9->DrawPrimitive( D3DPT_TRIANGLEFAN, First, NumPts - 2 );
	...
	void __fastcall SetTextureClampMode( INT Mode )
	{
		if(TextureClampMode!=Mode)
		{
			...
			Direct3DDevice9->SetSamplerState( 0, D3DSAMP_ADDRESSU, TextureMode );
			Direct3DDevice9->SetSamplerState( 0, D3DSAMP_ADDRESSV, TextureMode );
		}
	}
	...
	)
	{
		...
		if( Info.Palette && Info.Palette[128].A!=255 && !(PolyFlags&PF_Translucent) && !(PolyFlagsEx&PFX_Translucent2))
		...
		Direct3DDevice9->DrawPrimitive( D3DPT_TRIANGLEFAN, First, 2 );
	...
	void __fastcall QueueParticleFlush(AParticleSystem &System, FSceneNode *Frame)
	{
		...
		if(VariableAlpha && !(PolyFlagsEx&PFX_Translucent2)) PolyFlags|=PF_Translucent;
		
		SetBlending(PolyFlags,PolyFlagsEx);

		// Set the zbias for the entire particle system:
		SetZBias(System.ZBias); //Direct3DDevice9->SetRenderState(D3DRS_ZBIAS,System.ZBias);
		SetTextureNULL(1);
		...
		for(i=0;i<QueuedParticles.Num();i++)
		{
			...
			Direct3DDevice9->DrawPrimitive( D3DPT_TRIANGLELIST, First+(ParticleIndex*6), RunLength*2);
	...
	)
	{
		...
		if((StartWidth==1.f)&&(EndWidth==1.f))
		{
			...
			Direct3DDevice9->DrawPrimitive( D3DPT_LINELIST, First, 1 );
			Stats.Particles++;
	...
	UBOOL Exec( const TCHAR* Cmd, FOutputDevice& Ar )
	{
		...
		else if( ParseCommand(&Cmd,TEXT("LodBias")) )
		{
			...
			Direct3DDevice9->SetSamplerState( 0, D3DSAMP_MIPMAPLODBIAS, *(DWORD*)&LodBias );
			Direct3DDevice9->SetSamplerState( 1, D3DSAMP_MIPMAPLODBIAS, *(DWORD*)&LodBias );
			return 1;
		}
	...
	void __fastcall Queued3DLinesFlush(FSceneNode* Frame) 
	{
		...
		//!!should optimize to avoid changing shade mode, color op, alpha op.
		//Direct3DDevice9->SetRenderState( D3DRS_SHADEMODE, D3DSHADE_FLAT );
		Direct3DDevice9->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_DISABLE  );
		Direct3DDevice9->SetTextureStageState( 0, D3DTSS_ALPHAOP, D3DTOP_DISABLE );
		...
		Direct3DDevice9->DrawPrimitive( D3DPT_LINELIST, First, Count/2 );
		// Line code:
	...
	void __fastcall UD3DRenderDevice::Draw2DPoint( FSceneNode* Frame, FPlane Color, DWORD LineFlags, FLOAT X1, FLOAT Y1, FLOAT X2, FLOAT Y2, FLOAT Z )
	{
		...
		Direct3DDevice9->DrawPrimitive( D3DPT_LINESTRIP, First, 4 );
	...
	void __fastcall UD3DRenderDevice::PushHit( const BYTE* Data, INT Count )
	{
		...
		if(FAILED(h=Direct3DDevice9->GetRenderTarget(0,&RenderTarget)))
		{
			debugf(TEXT("D3D Driver: GetRenderTarget failed (%s)"),DXGetErrorString(h));
			return;
		}
		...
		{
			debugf(TEXT("D3D Driver: LockRect failed (%s)"),DXGetErrorString(h));
			return;
		}
	...
	void __fastcall ClearZ( FSceneNode* Frame )
	{
		...
		Direct3DDevice9->Clear( 0, NULL, D3DCLEAR_ZBUFFER|D3DCLEAR_STENCIL, 0, 1.0, 0 );
	}
	...
	void __fastcall ReadPixels( FColor* Pixels, UBOOL BackBuffer = false)
	{
		...
		IDirect3DSurface9*	TempScreenBuffer;
	
		if (!BackBuffer)
		{
			// Create the temp surface to hold the front buffer
			h = Direct3DDevice9->CreateOffscreenPlainSurface(ViewportX, ViewportY, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM, &TempScreenBuffer, 0);
			if (FAILED(h))
			{
				D3D_CHECK(Direct3DDevice9->CreateOffscreenPlainSurface(ViewportX, ViewportY, D3DFMT_A8R8G8B8, D3DPOOL_SCRATCH, &TempScreenBuffer, 0));
			}

			// Fill the temporary surface with the contents of the front buffer.
			D3D_CHECK((h=Direct3DDevice9->GetFrontBufferData(0, TempScreenBuffer)));
		}
		else
		{
			EndScene();
			...
			D3D_CHECK((h=Direct3DDevice9->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &TempScreenBuffer)));
		}
		...
		if(DeviceCaps9.Caps2 & D3DCAPS2_FULLSCREENGAMMA)
		{
	...
	void __fastcall UD3DRenderDevice::EndFlash()
	{
		...
		if( FlashScale!=FVector(.5f,.5f,.5f) || FlashFog!=FVector(0,0,0) )
		{
			...
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_DISABLE ); // v 0.4
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_ALPHAOP, D3DTOP_SELECTARG2 );
			D3DCMPFUNC OriginalZFunc = SetZFunc(D3DCMP_ALWAYS);

			Direct3DDevice9->SetFVF(FD3DScreenVertex::FVF);
			Direct3DDevice9->DrawPrimitiveUP( D3DPT_TRIANGLEFAN, 2, Vertices, sizeof(FD3DScreenVertex) );
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_MODULATE );
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_ALPHAOP, D3DTOP_MODULATE );
			SetZFunc(OriginalZFunc);
			SetBlending();
	...
	FLOAT __fastcall SetZBias( FLOAT NewZBias=0.f )
	{
		...
		if(ZBias!=NewZBias || !CacheBlending)
		{
			...
			// Raziel: Direct3D 8 used integer values between 0 and 16 for Z bias. Direct3D 9
			//  expects a FLOAT (from -1 to +1) which has been cast to a DWORD. In practice,
			//  this means we have to convert the old Z bias value to a small negative FLOAT,
			//  then cast it to a DWORD before passing it to SetRenderState:
			FLOAT convertedZBias = ZBias * -0.000001f;
			DWORD* valueToSet = (DWORD*) & convertedZBias;
			Direct3DDevice9->SetRenderState(D3DRS_DEPTHBIAS, *valueToSet);
		}

		return FormerZBias;
	}

	BOOL __fastcall SetZWriteEnable(BOOL NewZWriteEnable = TRUE)
	{
		BOOL FormerZWriteEnable = ZWriteEnable;

		if (NewZWriteEnable != FormerZWriteEnable || !CacheBlending)
		{
			Direct3DDevice9->SetRenderState(D3DRS_ZWRITEENABLE, NewZWriteEnable);
			ZWriteEnable = NewZWriteEnable;
		}

		return FormerZWriteEnable;
	}

	D3DCMPFUNC __fastcall SetZFunc(D3DCMPFUNC NewZFunc = D3DCMP_LESSEQUAL)
	{
		D3DCMPFUNC FormerZFunc = ZFunc;

		if (NewZFunc != FormerZFunc || !CacheBlending)
		{
			Direct3DDevice9->SetRenderState(D3DRS_ZFUNC, NewZFunc);
			ZFunc = NewZFunc;
		}
	...
	D3DBLEND __fastcall SetSrcBlend( D3DBLEND NewSrcBlend = D3DBLEND_ZERO )
	{
		...
		if(NewSrcBlend!=FormerSrcBlend || !CacheBlending)
		{
			...
			Direct3DDevice9->SetRenderState( D3DRS_SRCBLEND,  NewSrcBlend );
	...
	D3DBLEND __fastcall SetDstBlend( D3DBLEND NewDstBlend = D3DBLEND_ZERO )
	{
		...
		if(NewDstBlend!=FormerDstBlend || !CacheBlending)
		{
			...
			Direct3DDevice9->SetRenderState( D3DRS_DESTBLEND,  NewDstBlend );
	...
	INT __fastcall SetAlphaBlendEnable(INT NewAlphaBlendEnable)
	{
		...
		{
			Direct3DDevice9->SetRenderState( D3DRS_ALPHABLENDENABLE, NewAlphaBlendEnable );
	...
	void __fastcall SetBlending( DWORD PolyFlags=0, DWORD PolyFlagsEx=0 )
	{
		...
		if( (!(PolyFlags & (PF_Translucent|PF_Modulated))&&(!(PolyFlagsEx & (PFX_AlphaMap|PFX_LightenModulate|PFX_DarkenModulate|PFX_Translucent2)))) )
			PolyFlags |= PF_Occlude;
		else if( PolyFlags & PF_Translucent || PolyFlagsEx & PFX_Translucent2 )
			PolyFlags &= ~PF_Masked;
		...
		if(Xor&PF_TwoSided)
		{
			Direct3DDevice9->SetRenderState( D3DRS_CULLMODE,(PolyFlags&PF_TwoSided)?D3DCULL_NONE:D3DCULL_CCW);
		}

		if(XorEx&PFX_Clip)
		{
			Direct3DDevice9->SetRenderState( D3DRS_CLIPPING, (bool)(PolyFlagsEx&PFX_Clip));
		}

		if(XorEx&PFX_FlatShade)
		{
			Direct3DDevice9->SetRenderState( D3DRS_SHADEMODE, (PolyFlagsEx&PFX_FlatShade)?D3DSHADE_FLAT:D3DSHADE_GOURAUD );
		}
		  ...
		  ||(XorEx& (PFX_AlphaMap|PFX_LightenModulate|PFX_DarkenModulate|PFX_Translucent2)))
		{
			if( Xor&(PF_Invisible|PF_Translucent|PF_Modulated|PF_Highlighted) || (XorEx&(PFX_AlphaMap|PFX_LightenModulate|PFX_DarkenModulate|PFX_Translucent2)))
			{
				...
				{
					Direct3DDevice9->SetRenderState( D3DRS_ALPHAREF, 127);
				...
				else if( PolyFlagsEx & PFX_AlphaMap )
				{
					...
					Direct3DDevice9->SetRenderState( D3DRS_ALPHAREF, 8);
					Direct3DDevice9->SetRenderState( D3DRS_ALPHAFUNC, D3DCMP_GREATER );
					Direct3DDevice9->SetRenderState( D3DRS_ALPHATESTENABLE, 1 );

					Direct3DDevice9->SetTextureStageState( 0, D3DTSS_ALPHAOP,   D3DTOP_SELECTARG1);
			...
			if( Xor & PF_Occlude )
			{
				SetZWriteEnable((PolyFlags& PF_Occlude) != 0);
			}
			if( Xor & PF_Masked )
			{
				if( PolyFlags&PF_Masked )
				...
				{
					Direct3DDevice9->SetRenderState( D3DRS_ALPHATESTENABLE, 0 );
				}
			}
			if( Xor & PF_NoSmooth )
			{
				Direct3DDevice9->SetSamplerState( 0, D3DSAMP_MAGFILTER, (PolyFlags & PF_NoSmooth) ? D3DTEXF_POINT : D3DTEXF_LINEAR );
				Direct3DDevice9->SetSamplerState( 0, D3DSAMP_MINFILTER, (PolyFlags & PF_NoSmooth) ? D3DTEXF_POINT : D3DTEXF_LINEAR );
			}
			if( Xor & PF_RenderFog )
			{
				Direct3DDevice9->SetRenderState( D3DRS_SPECULARENABLE, (PolyFlags&PF_RenderFog)!=0 );
			}
			if( (Xor & PF_Memorized) || (Xor & PF_Selected) )
			{
				...
				{
					// Lightmap
					Direct3DDevice9->SetTextureStageState( 1, D3DTSS_COLOROP, D3DTOP_MODULATE );
					Direct3DDevice9->SetTextureStageState( 1, D3DTSS_ALPHAOP, D3DTOP_SELECTARG2 );
				}
				else
				if( PolyFlags&PF_Selected )
				{
					// Alphamap
					Direct3DDevice9->SetTextureStageState( 1, D3DTSS_COLOROP, D3DTOP_SELECTARG2 );
					Direct3DDevice9->SetTextureStageState( 1, D3DTSS_ALPHAOP, D3DTOP_SELECTARG1 );
				}
				else
				{
					Direct3DDevice9->SetTextureStageState( 1, D3DTSS_COLOROP, D3DTOP_DISABLE );
	...
	void __forceinline ReleaseOldestTexture()
	{
		...
		while(CachedTextures && (CachedTextures->Filler->PixelFormat->BinnedRAM>Threshold))
		{
			...
			if(LowestFrameCount->Texture9)
				SafeRelease(LowestFrameCount->Texture9);
	...
	void __forceinline __fastcall SetTextureNULL( DWORD dwStage )
	{
		...
		Direct3DDevice9->SetTexture( dwStage, NULL );
	...
	void __fastcall SetTexture( DWORD dwStage, FTextureInfo& Info, DWORD PolyFlags=0, UBOOL Precache=FALSE, DWORD PolyFlagsEx=0, UBOOL IsLightmap=FALSE )
	{
		...
		if(!TexInfo)
		{
			...
			while(Info.Mips[FirstMip]->USize > (INT) DeviceCaps9.MaxTextureWidth || Info.Mips[FirstMip]->VSize > (INT) DeviceCaps9.MaxTextureHeight)
			...
			D3D_CHECK((h=Direct3DDevice9->CreateTexture(USize,VSize,Info.NumMips - FirstMip,0,Filler->PixelFormat->Direct3DFormat,
				D3DPOOL_MANAGED,&TexInfo->Texture9,NULL)));
		...
		if( Info.bRealtimeChanged /*&& Info.bParametric || (Info.Format==TEXF_RGBA7 && GET_COLOR_DWORD(*Info.MaxColor)==0xFFFFFFFF)*/ )
		{
			...
			for( INT MipIndex=TexInfo->FirstMip, ListIndex=0; ListIndex<Count; ListIndex++,MipIndex++ )
			{
				...
				TexInfo->Texture9->GetLevelDesc(ListIndex,&SurfaceDesc);
				TexInfo->Texture9->LockRect(ListIndex,&LockedRect,NULL,0);
				...
				TexInfo->Texture9->UnlockRect(ListIndex);
			}
			Stats.TexUploads++;
		...
		// Set Direct3D state.
		Direct3DDevice9->SetTexture(dwStage,TexInfo->Texture9);

		// Raziel: Enabling mipmapping seems to break the alpha channel, 3D Realms ran into the same issue with PFX_AlphaMap,
		//  I've moved their code from SetBlending() to SetTexture() so mipmap settings is now handled in a single location.
		//  Mipmap settings being handled in multiple locations was actually the cause of the hair corruption when multiple
		//  characters were on sceen since SetTexture() would actually override the settings from SetBlending() if multiple
		//  sets of hair is rendered one after the other.
		if (!TexInfo->UseMips | (PolyFlags & PF_Translucent) | (PolyFlagsEx & (PFX_AlphaMap|PFX_Translucent2)))
			Direct3DDevice9->SetSamplerState(dwStage, D3DSAMP_MIPFILTER, D3DTEXF_NONE);
		else if (!Stages[dwStage] || Stages[dwStage]->UseMips)
			Direct3DDevice9->SetSamplerState(dwStage, D3DSAMP_MIPFILTER, UseTrilinear ? D3DTEXF_LINEAR : D3DTEXF_POINT);
	...
	UBOOL __fastcall SetRes( INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen )
	{
		...
		verify(this);
		verify(Direct3D9);
		verify(NewColorBytes==4 || NewColorBytes == 2);

		D3DFORMAT FormatsToSeek2[] = { D3DFMT_R5G6B5, D3DFMT_X1R5G5B5, D3DFMT_A1R5G5B5 };
		D3DFORMAT FormatsToSeek4[] = { D3DFMT_A8R8G8B8, D3DFMT_X8R8G8B8 };
		D3DFORMAT *FormatsToSeek = (NewColorBytes == 2) ? FormatsToSeek2 : FormatsToSeek4;
		INT NumFormatsToSeek = (NewColorBytes == 2) ? 3 : 2;
		...
		for (DWORD FormatIndex = 0; FormatIndex < NumFormatsToSeek; FormatIndex++)
		{
			D3DFORMAT D3DFormat = FormatsToSeek[FormatIndex];
			for (DWORD ModeIndex = 0; ModeIndex < Direct3D9->GetAdapterModeCount(BestAdapterIndex, D3DFormat); ModeIndex++)
			{
				D3DDISPLAYMODE DisplayMode;
				Direct3D9->EnumAdapterModes(BestAdapterIndex, D3DFormat, ModeIndex, &DisplayMode);
		...
		if(Fullscreen)
		{
			...
			for(INT Index = 0;Index < DisplayModes.Num();Index++)
			{
				...
				// Raziel TODO: Check the logic below to see if it is correct for D3D9
		...
		else
		{
			...
			if(FAILED(h=Direct3D9->GetAdapterDisplayMode(BestAdapterIndex,&DisplayMode)))
		...
		PresentParms.PresentationInterval = Fullscreen ? (UseVSync ? D3DPRESENT_INTERVAL_ONE : D3DPRESENT_INTERVAL_IMMEDIATE) : D3DPRESENT_INTERVAL_DEFAULT;
		...
		while(Direct3D9->CheckDeviceFormat(BestAdapterIndex,D3DDEVTYPE_HAL,AdapterFormat,D3DUSAGE_RENDERTARGET,D3DRTYPE_SURFACE,BackBufferFormat) != D3D_OK)
		{
			...
			else return UnSetRes(TEXT("CheckDeviceFormat: Could not find suitable back buffer format"),0);
		...
		while(Direct3D9->CheckDeviceFormat(BestAdapterIndex,D3DDEVTYPE_HAL,AdapterFormat,D3DUSAGE_DEPTHSTENCIL,D3DRTYPE_SURFACE,DepthFormat) != D3D_OK ||
			  Direct3D9->CheckDepthStencilMatch(BestAdapterIndex,D3DDEVTYPE_HAL,AdapterFormat,BackBufferFormat,DepthFormat) != D3D_OK)
		{
				...
				return UnSetRes(TEXT("CheckDepthStencilMatch: Could not find suitable depth format"),0);
		}
		...
		if(Direct3DDevice9)
		{
			...
			//SafeRelease(Direct3DDevice9);

			D3D_CHECK(Direct3DDevice9->Reset(&PresentParms));
		} 

		if(!Direct3DDevice9)
		{ 
			// Create the Direct3D device.
			if(FAILED(h=Direct3D9->CreateDevice(BestAdapterIndex,D3DDEVTYPE_HAL,ViewporthWnd,D3DCREATE_HARDWARE_VERTEXPROCESSING,&PresentParms,&Direct3DDevice9)))
			{
				debugf(TEXT("ViewporthWnd:%08x"),ViewporthWnd);
				debugf(TEXT("Failed to set hardware vertex processing: %s, attempting to set software vertexprocessing"),DXGetErrorString(h));
				// If hardware vertex processing failed, switch to software:
				if(FAILED(h=Direct3D9->CreateDevice(BestAdapterIndex,D3DDEVTYPE_HAL,ViewporthWnd,D3DCREATE_SOFTWARE_VERTEXPROCESSING,&PresentParms,&Direct3DDevice9)))
		...
		verify(!FAILED(Direct3DDevice9->SetViewport(&ViewportInfo)));
		verify(!FAILED(Direct3DDevice9->GetViewport(&ViewportInfo)));
		...
		// Handle the texture formats we need.
		{
			...
			if(!FAILED(Direct3D9->CheckDeviceFormat(BestAdapterIndex,D3DDEVTYPE_HAL,AdapterFormat,0,D3DRTYPE_TEXTURE,D3DFMT_A8R8G8B8)))
				RecognizePixelFormat(Format8888,D3DFMT_A8R8G8B8,TEXT("A8R8G8B8"));
						
			if(!FAILED(Direct3D9->CheckDeviceFormat(BestAdapterIndex,D3DDEVTYPE_HAL,AdapterFormat,0,D3DRTYPE_TEXTURE,D3DFMT_A1R5G5B5)))
				RecognizePixelFormat(Format1555,D3DFMT_A1R5G5B5,TEXT("A1R5G5B5"));
		}

		// Verify mipmapping supported.
		if(!(DeviceCaps9.TextureFilterCaps & D3DPTFILTERCAPS_MIPFPOINT)
		...
		// Check caps.
		if( DeviceCaps9.ShadeCaps & D3DPSHADECAPS_SPECULARGOURAUDRGB )
		...
		if(	DeviceCaps9.RasterCaps & D3DPRASTERCAPS_WBUFFER)
		{
			...
			{
				Direct3DDevice9->SetRenderState( D3DRS_ZENABLE, D3DZB_USEW );
		...
		// Init render states.
		{
			...
			SetZFunc(D3DCMP_LESSEQUAL);
			Direct3DDevice9->SetRenderState( D3DRS_FOGCOLOR, 0 );        
			Direct3DDevice9->SetRenderState( D3DRS_FOGTABLEMODE, D3DFOG_LINEAR );
			FLOAT FogStart=0.f, FogEnd = 65535.f;
			Direct3DDevice9->SetRenderState( D3DRS_FOGSTART, *(DWORD*)&FogStart );
			Direct3DDevice9->SetRenderState( D3DRS_FOGEND, *(DWORD*)&FogEnd );
			...
			memset(&Material9,0,sizeof(Material9));

			Material9.Ambient.r = 1.0f; Material9.Ambient.g = 1.0f; Material9.Ambient.b = 1.0f; Material9.Ambient.a = 1.0f;
			Material9.Diffuse.r = 0.5f; Material9.Diffuse.g = 0.5f; Material9.Diffuse.b = 0.5f; Material9.Diffuse.a = 1.0f;
			Material9.Power = 0.f;

			Direct3DDevice9->SetMaterial(&Material9);
			Direct3DDevice9->SetRenderState(D3DRS_SHADEMODE,D3DSHADE_GOURAUD);
			...
			// Raziel: Blend state needs to be set to a normal state here or you'll get a black screen when changing resolutions
			SetBlending(0, 0);
		}
		...
		// Init texture stage state.
		{
			...
			//FLOAT LodBias=-0.5f;
			Direct3DDevice9->SetSamplerState( 0, D3DSAMP_MIPMAPLODBIAS, *(DWORD*)&LodBias );
			Direct3DDevice9->SetSamplerState( 0, D3DSAMP_ADDRESSU,   D3DTADDRESS_WRAP );
			Direct3DDevice9->SetSamplerState( 0, D3DSAMP_ADDRESSV,   D3DTADDRESS_WRAP );
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_COLORARG1, D3DTA_TEXTURE );
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_COLORARG2, D3DTA_DIFFUSE );
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_COLOROP,   D3DTOP_MODULATE );
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE );
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_ALPHAARG2, D3DTA_DIFFUSE );
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_ALPHAOP,   D3DTOP_MODULATE );
			Direct3DDevice9->SetSamplerState( 0, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR );
			Direct3DDevice9->SetSamplerState( 0, D3DSAMP_MINFILTER, D3DTEXF_LINEAR );
			Direct3DDevice9->SetSamplerState( 0, D3DSAMP_MIPFILTER, (UseTrilinear ? D3DTEXF_LINEAR : D3DTEXF_POINT));
			Direct3DDevice9->SetTextureStageState( 0, D3DTSS_TEXCOORDINDEX, 0 );
			...
			Direct3DDevice9->SetSamplerState( 1, D3DSAMP_ADDRESSV,   D3DTADDRESS_WRAP );
			Direct3DDevice9->SetTextureStageState( 1, D3DTSS_COLORARG1, D3DTA_TEXTURE );
			Direct3DDevice9->SetTextureStageState( 1, D3DTSS_COLORARG2, D3DTA_CURRENT );
			Direct3DDevice9->SetTextureStageState( 1, D3DTSS_COLOROP,   D3DTOP_DISABLE );
			Direct3DDevice9->SetTextureStageState( 1, D3DTSS_ALPHAARG1, D3DTA_TEXTURE );
			Direct3DDevice9->SetTextureStageState( 1, D3DTSS_ALPHAARG2, D3DTA_CURRENT );
			Direct3DDevice9->SetTextureStageState( 1, D3DTSS_ALPHAOP,   D3DTOP_DISABLE );
			Direct3DDevice9->SetSamplerState( 1, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR );
			Direct3DDevice9->SetSamplerState( 1, D3DSAMP_MINFILTER, D3DTEXF_LINEAR );
			Direct3DDevice9->SetSamplerState( 1, D3DSAMP_MIPFILTER, UseTrilinear ? D3DTEXF_LINEAR : D3DTEXF_POINT  );
			Direct3DDevice9->SetTextureStageState( 1, D3DTSS_TEXCOORDINDEX, 1 );
		}

		// Update the viewport.
		verify(Viewport->ResizeViewport((Fullscreen ? BLIT_Fullscreen : 0) | BLIT_Direct3D, ViewportX, ViewportY, ViewportColorBits / 8 ));
		...
		WorldVertices.Init(Direct3DDevice9,WORLDSURFACE_VERTEXBUFFER_SIZE);
		ActorVertices.Init(Direct3DDevice9,ACTORPOLY_VERTEXBUFFER_SIZE);
		LineVertices.Init(Direct3DDevice9,LINE_VERTEXBUFFER_SIZE);
		ParticleVertices.Init(Direct3DDevice9,PARTICLE_VERTEXBUFFER_SIZE);
		...
		// Determine which rendertarget format to use, and create clipper texture
		{
			...
			Direct3D9->GetAdapterDisplayMode(BestAdapterIndex,&DisplayMode);
			...
			if (Direct3D9->CheckDeviceFormat(BestAdapterIndex,D3DDEVTYPE_HAL,AdapterFormat,D3DUSAGE_RENDERTARGET,D3DRTYPE_SURFACE,RenderTargetFormat) != D3D_OK)
			...
			if (RenderTargetFormat != D3DFMT_UNKNOWN)
			{
				...
				if (Direct3DDevice9->CreateTexture(128,1,1,0,D3DFMT_R5G6B5,D3DPOOL_MANAGED,&ClipperTexture,NULL) == D3D_OK)
				{
	...
	UBOOL __fastcall UnSetRes( const TCHAR* Msg, HRESULT h )
	{
		...
		if( Msg ) debugf(NAME_Init,TEXT("%s (%s)"),Msg,DXGetErrorString(h));
		return 0;
	}
	...
	{	
		...
		{		
			// Enable fog.
			Direct3DDevice9->SetRenderState(D3DRS_FOGENABLE, TRUE);
			Direct3DDevice9->SetRenderState(D3DRS_RANGEFOGENABLE, TRUE);

			// Set the fog color.
			Direct3DDevice9->SetRenderState(D3DRS_FOGCOLOR, ((DWORD)(DistanceFogColor.R)<<16) | ((DWORD)(DistanceFogColor.G)<<8) | ((DWORD)(DistanceFogColor.B)));

			// Set fog parameters.
			Direct3DDevice9->SetRenderState(D3DRS_FOGTABLEMODE, D3DFOG_LINEAR);
			Direct3DDevice9->SetRenderState(D3DRS_FOGSTART, *(DWORD *)(&DistanceFogBegin));
			Direct3DDevice9->SetRenderState(D3DRS_FOGEND,   *(DWORD *)(&DistanceFogEnd));
		}
		else
			Direct3DDevice9->SetRenderState(D3DRS_FOGENABLE, FALSE);		
	...
	UBOOL InitRenderTargetRes(RenderTargetInfo *RT)
	{
		...
		if (Direct3DDevice9->CreateTexture(RT->Width,RT->Height,1,D3DUSAGE_RENDERTARGET,RenderTargetFormat,D3DPOOL_DEFAULT,&RT->pRenderTargetTex,NULL) != D3D_OK)
			return false;
	...
	void DestroyRenderTarget(void **pRenderTarget) 
	{
		...
		{
			debugf(TEXT("DestroyRenderTarget: Invalid RenderTarget=%d", Index));
			// FairFriend: Exit gracefully. Happens when RenDev is re-init but UnRender doesn't know it.
			return;
		}
...

	{
		...
		if (Index < 0)
			appErrorf(TEXT("SetRenderTarget: Invalid RenderTarget=%d", Index));

		if (Index >= RenderTargetArray.Num())
		{
			debugf(TEXT("SetRenderTarget: RenderTarget Index >= RenderTargetArray.Num %d", RenderTargetArray.Num()));

			// FairFriend: Destroy old render targets.	
			for (INT i = 0; i < RenderTargetArray.Num(); i++)
			{
				ShutdownRenderTargetRes(&RenderTargetArray(i));
				RenderTargetArray(i).Active = false;
				FreeRenderTargets.AddItem(Index);
			}
			// FairFriend: Create missing rener targets.			
			GRender->ShadowRenderTargetsCreated = false;
			debugf(TEXT("SetRenderTarget: ShadowRenderTargetsCreated %i "), GRender->ShadowRenderTargetsCreated);
			GRender->CreateShadowRenderTargets(Viewport, GRender->GMaxFinalShadowActors);
			return;
		}
		...
		if (!pOriginalZStencil)
		{
			...
			Direct3DDevice9->GetRenderTarget(0, &pOriginalRenderTarget);
			Direct3DDevice9->GetDepthStencilSurface(&pOriginalZStencil);
		}

		if (!RenderTargetArray(Index).pRenderTargetSurf)
			appErrorf(TEXT("SetRenderTarget: NULL RenderTargetSurf"));

		// Set the render target
		Direct3DDevice9->SetRenderTarget(0, RenderTargetArray(Index).pRenderTargetSurf);
		Direct3DDevice9->SetDepthStencilSurface(NULL);
		
		// Clear the render target
		Direct3DDevice9->Clear(0, NULL, D3DCLEAR_TARGET, 0xffffffff, 1.f, 0 );
		/*
		D3DRECT		Rect;
		...
		Direct3DDevice9->Clear(1, &Rect, D3DCLEAR_TARGET, 0, 1.f, 0 );
		*/
	...
	void RestoreRenderTarget(void)
	{
		...
		Direct3DDevice9->SetRenderTarget(0, pOriginalRenderTarget);
		Direct3DDevice9->SetDepthStencilSurface(pOriginalZStencil);
							...
							IDirect3DTexture9	*Tex)
	{
		...
		Direct3DDevice9->SetTexture(0, Tex);
	...
	void __fastcall AddProjector(FSceneNode *Frame, void *pRenderTarget, FTextureInfo *Info, FLOAT wNear, FLOAT wFar, FLOAT FadeScale)
	{
		...
		if (pRenderTarget)
		{
			...
			if (Index < 0)
				appErrorf(TEXT("AddProjector: Invalid RenderTarget=%d", Index));

			if (Index >= RenderTargetArray.Num())
			{
				appErrorf(TEXT("AddProjector: RenderTarget Index >= RenderTargetArray.Num %d", RenderTargetArray.Num()));
			}
		...
		// Render tex into itself, have convolution smooth texture out
			...
			{
				// FairFriend: Update resolution of texture the shadow is projected on.
				float res = GRender->iShadowRes;

				if (!TempRT)
					TempRT = CreateRenderTarget(res, res);
				...
				if (TempRT)
				{
					...
					// FairFriend: Shadow resolution has changed, destroy old render target and create a new one
					if (RT2.Height != res)
					{
						DestroyRenderTarget(&TempRT);
						TempRT = CreateRenderTarget(res, res);
						
						Index2 = RENDER_TARGET_TO_INDEX(TempRT);
						if (Index2 < 0 || Index2 >= RenderTargetArray.Num())
							appErrorf(TEXT("AddProjector: Invalid RenderTarget2"));
						RenderTargetInfo& RT2 = RenderTargetArray(Index2);
					}
					...
					DrawTex(Frame, 0.0f, 0.0f, res - 1.0f, res - 1.0f,Val*0.5f,Val*0.5f, 1.0f, 1.0f, RT.pRenderTargetTex);
					SetRenderTarget(pRenderTarget, NULL);
					DrawTex(Frame, 0.0f, 0.0f, res - 1.0f, res - 1.0f,-Val*0.5f,-Val*0.5f, 1.0f, 1.0f, RT2.pRenderTargetTex);
				}
			}
		#else
			if (CurrentRenderTarget == pRenderTarget)
