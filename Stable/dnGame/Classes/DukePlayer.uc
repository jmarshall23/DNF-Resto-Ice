...
var texture					NightVisionVignette;
...
var unbound int				CurrentPickupSound; // -x
var unbound float			NextMirrorEgoTime;
var unbound float			NextTalkTime;
var unbound float			NextPickupTalkTime; // -x
...
var sound					HitNotificationSound;
var globalconfig string		HitNotificationSoundName;
...
{
	// maybe damage type would be a better check? -x
	if ( HeadRegion.Zone.bWaterZone )
	{
		PlayOwnedSound( PlayerReplicationInfo.VoiceType.default.UnderWaterPain, SLOT_Pain,,,, Frand() * 0.2 + 0.9 );
		//DukeVoice.DukeSay( PlayerReplicationInfo.VoiceType.default.UnderWaterPain, Frand() * 0.2 + 0.9 );
		return;
	}
	PlayOwnedSound( PlayerReplicationInfo.VoiceType.default.DeathSounds[Rand(PlayerReplicationInfo.VoiceType.default.NumDeathSounds)], SLOT_Talk,,,,, true );
	PlayOwnedSound( PlayerReplicationInfo.VoiceType.default.DeathSounds[Rand(PlayerReplicationInfo.VoiceType.default.NumDeathSounds)], SLOT_Pain,,,,, true);

	// not using duke voice so it doesnt broadcast, but maybe it's fine to do so? -x
	//DukeVoice.DukeSay(PlayerReplicationInfo.VoiceType.default.DeathSounds[Rand(PlayerReplicationInfo.VoiceType.default.NumDeathSounds)]);
}
...
function PlayToWaiting( optional float TweenTime )
{
	...
	{
		if (bWeaponsActive && Weapon != None && !Weapon.IsA('MightyFoot'))
			PlayAllAnim('A_IdleStandActive',,0.5,false);
		else
			PlayAllAnim('A_IdleStandInactive',,0.5,false);
        PlayTopWeaponIdle();
    }
}
...
function PlayWaiting()
{
    ...
    {
		if (bWeaponsActive && Weapon != None && !Weapon.IsA('MightyFoot'))
			PlayAllAnim('A_IdleStandActive',,0.1,false);
		else
			PlayAllAnim('A_IdleStandInactive',,0.1,false);
        PlayTopWeaponIdle();
	}
}
...
exec function Give( string ClassName )
{
	local Inventory InventoryItem;
	local Weapon Weap;
	local class<Weapon> WeaponClass;
	...
	WeaponClass = class<Weapon>(DynamicLoadObject( ClassName, class'Class' ));
	InvClass = class<Inventory>(DynamicLoadObject( ClassName, class'Class' ));
	...
	if (WeaponClass != None)
	{
		...
		if ( Weap != None )
		{
			// just does 4 main ammo types, maybe there's a way to get count of ammo types? -x
			...
			Weap.AmmoType.ModeAmount[3] = Weap.AmmoType.MaxAmmo[3];
		}
	}
...
exec function AllAmmo()
{
	...
	Level.Game.GiveWeaponTo( self, WeaponClass );
...
exec function PissDown()
{
	...
	// Don't allow pissing as spectator ~hogsy
	if ( IsSpectating() )
		return;
...
event PlayerTick( float DeltaSeconds )
{
	...
	local vector X, Y, Z;
	local rotator PissRotation;
	local Sound HitNotificationSound;
	
	if ( /* ( Role < ROLE_Authority ) && */ ( HitCounter > OldHitCounter ) )
	{
		HitNotificationSound = Sound( DynamicLoadObject( HitNotificationSoundName, class'Sound' ) );
		
		if ( HitNotificationSound != None )
			PlaySound( HitNotificationSound, SLOT_Talk, 16 );
	}
	...
	// Check for pickups.
	// forgive the mess, i promise i'll improve it.. someday -x
	if ( CurrentPickupsIndex != RecentPickupsIndex )
	{
		if ( RecentPickupsIndex < CurrentPickupsIndex )
		{
			for ( i=CurrentPickupsIndex; i<6; i++ )
				if (ClassIsChildOf(RecentPickups[i], class'Weapon') || ClassIsChildOf(RecentPickups[i], class'Ammo'))
					WeaponLine(ClassIsChildOf(RecentPickups[i], class'Weapon'));
			for ( i=0; i<RecentPickupsIndex; i++ )
				if (ClassIsChildOf(RecentPickups[i], class'Weapon') || ClassIsChildOf(RecentPickups[i], class'Ammo'))
					WeaponLine(ClassIsChildOf(RecentPickups[i], class'Weapon'));
		}
		else 
		{
			for ( i=CurrentPickupsIndex; i<RecentPickupsIndex; i++ )
				if (ClassIsChildOf(RecentPickups[i], class'Weapon') || ClassIsChildOf(RecentPickups[i], class'Ammo'))
					WeaponLine(ClassIsChildOf(RecentPickups[i], class'Weapon'));
		}
		CurrentPickupsIndex = RecentPickupsIndex;
	}
...
function ClientPlayPainSound( class<DamageType> DamageType )
{
	...
	*/
	...
	if (FRand() < 0.5 && Level.TimeSeconds > NextTalkTime)
...
// x-
simulated function WeaponLine(optional bool bForce)
{
    // only in single player...?...yeah
    if (Level.Game.IsA('dnSinglePlayer') && (Level.NetMode == NM_Standalone))  
    {
		// 10% chance if ammo, 100% if weapon				30 second delay					no other one-liner playing
		if ((FRand() < 0.10 || bForce) && Level.TimeSeconds > NextPickupTalkTime && Level.TimeSeconds > NextTalkTime)
		{
			// 30 second delay for pickup one-liners
			NextPickupTalkTime = Level.TimeSeconds + 30 + GetSoundDuration(PlayerReplicationInfo.VoiceType.default.PickupSounds[CurrentPickupSound]);

			// TODO: randomize the initial line or pick a different one? 
			// since i noticed the first one always plays 
			// when you get the pistol in Apartments A -x
			DukeVoice.DukeSay( PlayerReplicationInfo.VoiceType.default.PickupSounds[CurrentPickupSound++] );
			
			if (CurrentPickupSound > PlayerReplicationInfo.VoiceType.default.NumPickupSounds-1)
				CurrentPickupSound = 0;
		}
	}
}
// -x
...
simulated function CheckMirror()
{
	...
	{
		// 5 second delay for mirror one-liners
		NextTalkTime = Level.TimeSeconds + 5 + GetSoundDuration(PlayerReplicationInfo.VoiceType.default.MirrorSounds[CurrentMirrorSound]);
		...
		if ( CurrentMirrorSound > PlayerReplicationInfo.VoiceType.default.NumMirrorSounds-1 )
...
simulated event RenderOverlays( canvas Canvas )
{
	...
	*/
    ...
    if ( (CameraStyle == PCS_HeatVision) && !bSniperZoomed )
	{
		...
		if ( bHelperMessages )
		{
			HelperString = "Press '"$DukeHUD(MyHUD).SpecialKeys[HUDToUse.ESpecialKeys.SK_Zoom]$"' to zoom.";
			Canvas.Style = 3;
	...
	if ( (CameraStyle == PCS_NightVision) && !bSniperZoomed )
	{
		...
		Canvas.Style = 4;
		Canvas.SetPos(0, 0);
		Canvas.DrawTile(Tex, Canvas.ClipX, Canvas.ClipY, 0, 0, Tex.USize, Tex.VSize, , , , true);

		// [JM] Draw Vignette
		// Shall I make this 4:3? 
		//Canvas.FillBorder(); // [JM] Fill edges.
		Tex = NightVisionVignette;
		Canvas.Style = 4;
		Canvas.SetPos(0, 0);
		Canvas.DrawTile(Tex, Canvas.ClipX, Canvas.ClipY, 0, 0, Tex.USize, Tex.VSize, , , , true);
		...
		Tex = NightVisionGlow;
		Canvas.Style = 3;
		Canvas.AC_SetPos(0, 0);
		Canvas.AC_DrawTile(Tex, Canvas.BaseX/2, Canvas.BaseY/2, 0, 0, Tex.USize, Tex.VSize, , , , true);
		Canvas.AC_SetPos(Canvas.BaseX/2, 0);
		Canvas.AC_DrawTile(Tex, Canvas.BaseX/2, Canvas.BaseY/2, 0, 0, Tex.USize, Tex.VSize, , , , true, , true );
		Canvas.AC_SetPos(0, Canvas.BaseY/2);
		Canvas.AC_DrawTile(Tex, Canvas.BaseX/2, Canvas.BaseY/2, 0, 0, Tex.USize, Tex.VSize, , , , true, , , true );
		Canvas.AC_SetPos(Canvas.BaseX/2, Canvas.BaseY/2);
		Canvas.AC_DrawTile(Tex, Canvas.BaseX/2, Canvas.BaseY/2, 0, 0, Tex.USize, Tex.VSize, , , , true, , true, true );
		*/
		...
		Canvas.DrawColor = DukeHUD(MyHUD).WhiteColor;
		Canvas.Style = 3; // [JM] Used to be 4
		Canvas.AC_SetPos(0, 0);
		Canvas.AC_DrawTile(Tex, Canvas.BaseX/2, Canvas.BaseY/2, 0, 0, Tex.USize, Tex.VSize, , , , true);
		Canvas.AC_SetPos(Canvas.BaseX/2, 0);
		Canvas.AC_DrawTile(Tex, Canvas.BaseX/2, Canvas.BaseY/2, 0, 0, Tex.USize, Tex.VSize, , , , true, , true );
	...
	else if ( (CameraStyle == PCS_ZoomMode) && !bSniperZoomed )
	{
		...
		Canvas.Style = 4;
		Canvas.AC_SetPos(0, 0);
		Canvas.AC_DrawTile(Tex, Canvas.BaseX, Canvas.BaseY, 0, 0, Tex.USize, Tex.VSize, , , , true);
		...
		Canvas.AC_SetPos( (Canvas.BaseX - XL)/2, 160 );
...
function DrawCompass(Canvas Canvas, float CompassPosition, float BaseY)
{
	...
	Canvas.AC_SetOrigin( Canvas.BaseX/2 - 100, 0.0 );
	DrawAdjustX -= Canvas.BaseX/2 - 100;

	// Adjust clipping region.
	OldClipX = Canvas.ClipX;
	OldClipY = Canvas.ClipY;
	Canvas.AC_SetClip( 200, Canvas.BaseY );
	...
	Canvas.AC_SetPos( (-RotFactor * RotLen + RotOffset + DrawAdjustX) - (XL/2), MedYPos );
	...
	Canvas.AC_SetPos( (-RotFactor * RotLen + RotOffset + DrawAdjustX) - (XL/2), SmallYPos );
...
function DrawCompassHash(Canvas Canvas, float HashPos, float RotLen, float RotOffset, float CompassPosition, float YPos, float DrawAdjustX)
{
	...
	if (RotFactor < 0.5)
		...
		RotFactor -= 1.0;
	Canvas.StrLen( "|", XL, YL );
	Canvas.AC_SetPos( (-RotFactor * RotLen + RotOffset + DrawAdjustX) - (XL/2), YPos );
...
// FairFriend: DukePlayer's implementation of SetPause.
function bool SetPause( BOOL bPause)
{
	// FairFriend: In multiplayer mode don't pause the game when taking a call
	//			   and let the animation and audio play out 
	if (Level.Netmode != NM_StandAlone && SOSstatus == SOS_CallInProgress)
		return Super.SetPause( bPause );		
		
	else if (Level.Netmode == NM_StandAlone && SOSstatus == SOS_CallInProgress)
	{
		if ( SOSInstigator != None )
			SOSInstigator.SetPause( bPause );
		return Super.SetPause( bPause );
	}
}
...
{	
	...
	//HitNotificationSound=Sound'dnGame.DukePlayer.HitNotificationSound'
	HitNotificationSoundName="dnGame.DukePlayer.HitNotificationSound"

	ZipperSound=sound'a_dukevoice.DukeLeak.DNZipper04'
	PissSound=sound'a_dukevoice.DukeLeak.DNLeak07'

    ZoomVisionSubtleStatic=Texture'hud_effects.subtledistort1.visionstaticD00'
    ZoomVisionGlow=Texture'hud_effects.zoomglow1BC'
	ZoomVisionOverlay=Texture'hud_effects.zoomblack1BC'
	...
	HeatVisionOverlay=Texture'hud_effects.heatblack1BC'
    HeatVisionGlow=Texture'hud_effects.heatglow1BC'

    //NightVisionOverlay=Texture'hud_effects.nightblack1BC' // [JM] Old one
	NightVisionOverlay=Texture'hud_effects.nightbordernew1BC'
    NightVisionGlow=Texture'hud_effects.nightglow1BC'
	NightVisionVignette=Texture'hud_effects.vignette1'
