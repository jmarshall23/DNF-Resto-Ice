...
simulated event RenderOverlays( canvas Canvas )
{
	...
	// [JM] Make sure our base res is set correctly.	
	Canvas.BaseX = 1024;
	Canvas.BaseY = 768;
	...
	if ( bZoomed )
	{
		...
		// Fill in the space around the screen.
		Canvas.FillBorder();
		...
		// Vertical
		P1 = Canvas.GetAspectOffset(Canvas.BaseX/2, 86, true);
		P2 = Canvas.GetAspectOffset(Canvas.BaseX/2, 683, true);
		...
		// Horizontal
		P1 = Canvas.GetAspectOffset(220, Canvas.BaseY/2, true);
		P2 = Canvas.GetAspectOffset(805, Canvas.BaseY/2, true);
		Canvas.DrawLine( P1, P2, false );
		// [JM] To-Do: Modify this so it draws as many lines times the pixel ratio difference.
		P1.Y += 1;
		P2.Y += 1;
		...
		// Top Left
		Canvas.AC_SetPos(256, 128);
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true);
		
		// Top Right
		Canvas.AC_SetPos(256+Tex.USize, 128); 
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true, , true );
		
		// Bottom Left
		Canvas.AC_SetPos(256, 128+Tex.VSize);
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true, , , true );
		
		// Bottom Right
		Canvas.AC_SetPos(256+Tex.USize, 128+Tex.VSize); 
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true, , true, true );

		// Draw the inner circle pieces. (2)
		Tex = SniperInnerCircle[1];

		// Top-Left
		Canvas.AC_SetPos(256, 0); 
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true);
		...
		// Bottom-Left
		Canvas.AC_SetPos(256, 768-Tex.VSize); 
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true, , , true );
		
		// Bottom-Right
		Canvas.AC_SetPos(Canvas.BaseX/2, 768-Tex.VSize); 
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true, , true, true );
		...
		// Side Upper-Left
		Canvas.AC_SetPos(128, 128);
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true);
		
		// Side Upper-Right
		Canvas.AC_SetPos(896-Tex.USize, 128); 
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true, , true);
		
		// Side Bottom-Left
		Canvas.AC_SetPos(128, 640-Tex.VSize); 
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true, , , true );
		
		// Side Bottom-Right
		Canvas.AC_SetPos(896-Tex.USize, 640-Tex.VSize); 
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true, , true, true );
		...
		Canvas.AC_SetPos(256, 42);
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, RotationScale*3.14159, (256-(Tex.USize/2)), (384 - 42 - (Tex.VSize/2)), true);
		
		// Transparent Ring (Half 2)
		Canvas.AC_SetPos(512, 42);
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, RotationScale*3.14159, (-Tex.USize/2), (384 - 42 - (Tex.VSize/2)), true, , true);
		...
		Canvas.AC_SetPos(256, 42);
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, RotationScale*3.14159 + PI, (256-(Tex.USize/2)), (384 - 42 - (Tex.VSize/2)), true, , , true);
		
		Canvas.AC_SetPos(512, 42);
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, RotationScale*3.14159 + PI, (-Tex.USize/2), (384 - 42 - (Tex.VSize/2)), true, , true, true);

		// Draw the uplink status.
		Tex = SniperUplink[1];	
		Canvas.AC_SetPos(31, 464);
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true);

		// Draw the zoom level.
		Tex = SniperZoomLevel[ZoomPower];
		Canvas.AC_SetPos(505, 510);
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true);

		// Draw the stats smack.
		Tex = SniperStats[1];
		Canvas.AC_SetPos(252, 146);
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true);

		// Draw the bacteria display.
		Tex = SniperBacteria;
		Canvas.AC_SetPos(11, 244);
		Canvas.AC_DrawTile(Tex, Tex.USize, Tex.VSize, 0, 0, Tex.USize, Tex.VSize, , , , true);
...
simulated function SpawnFirstPersonLaserBeam()
{
	...
	{
		GetAxes( PawnOwner.ViewRotation, X, Y, Z );
		DrawOffset = (CalcDrawOffset() * vect(0,0,1) + X*40 + Y*10 - Z*11) >> CalcDrawRotOffset(); //PawnOwner.BaseEyeHeight * vect(0,0,1) + X*40 + Y*10 - Z*11;
...
simulated function UpdateFlash( float DeltaTime )
{
	...
	FlashOffset = Pawn(Owner).Location + ((CalcDrawOffset() * vect(0,0,1) + X*40 + Y*10 - Z*11) >> CalcDrawRotOffset()); //Pawn(Owner).BaseEyeHeight*vect(0,0,1) + X*40 + Y*10 - Z*11;
