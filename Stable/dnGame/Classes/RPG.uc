...
var				bool			bCountingDown, bGreenForLaunch, bNukeLaunched;
var				SmackerTexture  NukeScreen;
var				Sound			NukeCountdownSound;
...
simulated function bool ClientFire()
{
	...
	if ( (AmmoType.AmmoMode == 1) && !bGreenForLaunch )
	{
		...
		// [JM] Will need to test if other players can hear this in MP.
		//      If not, we'll likely need to set up ourselves a proper fire state.
		//      Similar to how the shrink ray does for its charging sound.
		Owner.PlaySound( NukeCountdownSound, SLOT_Talk );
...
simulated function CycleAmmoMode( optional bool bFast )
{
	...
	if ( bNukeLaunched ) // [JM] Hack to make sure the RPG goes direct to idle after firing the nuke.
	{
		GotoState('Idle');
		bNukeLaunched = false;
	}
	else if ( (OldAmmoMode == 1) && (AmmoType.AmmoMode == 0) )
...
function WeaponAction( int ActionCode, rotator ClientViewRotation )
{
	if ( ActionCode == 0 )
	{
		...
		bNukeLaunched = true; // [JM] Hack to make sure the RPG goes to idle after firing the nuke.
		Fire();
...
defaultproperties
{
	...
	// [JM] Playing the sounds twice is a bit of a hack, but I had to. For some reason it plays too quiet otherwise.
	//      Need input from other devs.
	SAnimFire(0)=(AnimChance=1.000000,animSeq=Fire,AnimRate=1.000000,AnimSound=Sound'dnsWeapn.RPG.RPGFire01',AnimSound2=Sound'dnsWeapn.RPG.RPGFire01')
	SAnimFire(1)=(AnimChance=1.000000,animSeq=NukeFire,AnimRate=1.000000,AnimSound=Sound'dnsWeapn.RPG.RPGFire01',AnimSound2=Sound'dnsWeapn.RPG.RPGFire01')

    NotifySounds(0)=Sound'dnsWeapn.m16.M16Cock1'
	NotifySounds(1)=Sound'dnsWeapn.RPG.NukeActivate'
	NotifySounds(2)=Sound'dnsWeapn.RPG.NukeInsert'
	NotifySounds(3)=Sound'dnsWeapn.RPG.NukeScreenOpen'
	NotifySounds(4)=Sound'dnsWeapn.RPG.NukeRemove'
	NotifySounds(5)=Sound'dnsWeapn.RPG.NukeScreenClose'
	NukeCountdownSound=Sound'dnsWeapn.RPG.NukeCountdown'
