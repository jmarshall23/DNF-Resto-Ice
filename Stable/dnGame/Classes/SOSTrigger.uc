...
var	float				StartTime;			// FairFriend: Time at which the audio started playing
var float				ElapsedTime;		// FairFriend: How much time has passed since the audio started.
var float				SleepTime;			// FairFriend: Time spent in the sleep state.
...
// FairFriend: Un/Pause the audio.
function SetPause(bool bPause)
{	
	// Game is unpaused
	if ( !bPause) 
	{
		for (i=0; i< ArrayCount(SOSEvents); i++ )	
		{
			if ( SOSEvents[i].AnimActor != None) 
			{
				...
				// Add the portion of the audio already played to the delay
				DelayToUse += ElapsedTime;
			}		
		}
	...
	else
	{
		for (i=0; i< ArrayCount(SOSEvents); i++ )	
		{
			...
			{
				// Audio is stopped
				SOSEvents[i].AnimActor.StopSound( SLOT_Talk );
			}		
		}
		// Save how much time has passed since the audio started
		ElapsedTime = Level.TimeSeconds - StartTime;
	}
}
...
Begin:
	for (i=0; i< ArrayCount(SOSEvents); i++ )
	{
		...
		// FairFriend: Save the time at which the audio starts playing.
		StartTime = Level.TimeSeconds;
		while (DelayToUse >  0.0)
		{
			SleepTime = DelayToUse; 
			Sleep(DelayToUse);
			DelayToUse = DelayToUse - SleepTime; // FairFriend: Subtract to the delay the time
												 // 			spent in the sleep state
		}
	}
