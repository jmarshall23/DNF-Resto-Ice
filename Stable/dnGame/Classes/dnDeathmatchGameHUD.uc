...
simulated function PostRender( Canvas C )
{
    ...
    if ( PlayerOwner != None )
    {
		...
		if ( PlayerOwner.bClassicShowScores && PlayerOwner.Scoreboard != none ) 
			PlayerOwner.Scoreboard.DrawScores(C);
    }
...
simulated function DrawRemainingTime( Canvas C )
{
	...
	C.SetPos( (C.ClipX - XL) * 0.5, 0.025 * C.ClipY );	// ToDo: make sure it is perfectly centered
