...
var globalconfig bool bHideLookAt;
...
// SOS Messages.
var transient struct SOSMessageStruct
{
...
// -x
var bool isInteractable;
var globalconfig float HUDScale;
var bool ScaleChanged;
var float OldScale;
...
simulated function SetupScale(Canvas C)
{
	...
	*/
	...
	RootIndexTop 	= C.ClipY - (768.0 - 682.0) * HUDScaleY;
...
simulated function HUDSetup( canvas C )
{
	...
	// Update if scale changes too
	ScaleChanged = (HUDScale != OldScale);
	OldScale = HUDScale;

	// Reset the HUD scale if res or scale changed.
	if ( ResChanged || ScaleChanged )
		SetupScale( C );
...
//		ToolTip.HideWindow();
	...
	// Draw interaction prompt -x
	if ( !bIsSpectator )
		DrawInteractionPrompt( C );
...
simulated function DrawTitle( canvas C )
{
	...
	// just casually rewrote the entire rendering portion -x
	C.Font = font'HUDFont';
	C.bCenter = true;
	C.TextSize(Level.LevelEnterText, XL, YL, 1.0 * HUDScaleX, 1.0 * HUDScaleY);
	C.SetPos(0, 0.75 * C.ClipY);
	C.DrawText(Caps(Level.LevelEnterText),,,, 1.0 * HUDScaleX, 1.0 * HUDScaleY);

	C.TextSize(Level.LocationName, XL, YL, 0.75 * HUDScaleX, 0.75 * HUDScaleY);
	C.SetPos(0, 0.75 * C.ClipY + YL + (5 * HUDScaleY));
	C.DrawText(Caps(Level.LocationName),,,, 0.75 * HUDScaleX, 0.75 * HUDScaleY);
	C.bCenter = false;
...
simulated function DrawInteractionPrompt( canvas C )
{
	...
	isInteractable = LookActor != None && (LookActor.IsA('dnDecoration') || LookActor.IsA('Inventory') ||
		  LookActor.IsA('TriggerCrane') || LookActor.IsA('Mover') || LookActor.IsA('Carcass') ||
		  LookActor.IsA('Item') || LookActor.IsA('DoorHandle'));

		// Was a good option, but showing the current keybind is a little wacky -x
	/*
	// Perform the drawing.
	...
	{
		C.bCenter = true;
		//C.SetPos(0.5 * C.ClipX, 0.51 * C.ClipY);
		C.SetPos(0, 0.55 * C.ClipY);
		C.TextSize("E", XL, YL, 0.7*HUDScaleX, 0.7*HUDScaleY);
		C.DrawColor = HUDColor;
		C.DrawText("E",,,, 0.7*HUDScaleX, 0.7*HUDScaleY);
...
simulated function DrawBloodSmacks( canvas C )
{
	...
	BloodSmackSizeX = BloodSmackTexLeft.USize;
	BloodSmackSizeY = BloodSmackTexLeft.VSize;
	...
	for (i=0; i<4; i++)
	{
		...
		if (SmackTime > 0.0)
		{
			...
			XL = ((1.0-BloodSmacks[i].SmackSize)*StretchLen);
			YL = ((1.0-BloodSmacks[i].SmackSize)*StretchLen);
			if (i == 0)
			{
				// Left...
				C.SetPos( 0, 0 );
...
simulated function DrawLookAtInformation( canvas C )
{
	   ...
	   )
	{
		...
		if ( LookActor.IsA('Carcass') && Carcass(LookActor).bSearchable )
		{
			...
			C.SetPos( XOff, YOff-30*HUDScaleY+YL );
			...
			if ( dnCarcass(LookActor) != None )
			{
				...
				{
					C.DrawColor = HUDColor;
					C.SetPos( XOff + 70*HUDScaleX*i, YOff-32*HUDScaleY+YL+YL2 );
...
simulated function DrawStatusIndex( canvas C )
{
	...
	if ( TestRoot+YMod > C.ClipY )
		DesiredIndexTop = TestRoot - ((TestRoot+YMod) - (C.ClipY));
	else
...
simulated function DrawMiscBar( canvas C )
{
	...
	if ( MiscBarOut )
	{
		// Draw the misc bar.
		C.DrawColor = HUDColor;
		C.SetPos( XPos*HUDScaleX, IndexTop+19*HUDScaleY );
		Tex = MiscBarTexture;
		...
		if (false) // if (MiscBarExtended)
		{
			// Draw the misc bar extension.
			C.DrawColor = HUDColor;
			C.SetPos( XPos*HUDScaleX, IndexTop+19*HUDScaleY );
			Tex = MiscBarTexture;
			C.DrawScaledIcon( Tex, HUDScaleX, HUDScaleY );
...
simulated function DrawObjectives(canvas C)
{
	...
	for (i=0; i<6; i++)
	{
		...
		{
			C.TextSize( Level.ObjectiveInfos[i].Text, XL, YL, ObjectivesScale * HUDScaleX, ObjectivesScale * HUDScaleY );
			if ( XL > XL2 )
				XL2 = XL;
		}
	}
	if ( XL2 == 0.f )
		C.TextSize( "No Objectives Available", XL2, YL, ObjectivesScale * HUDScaleX, ObjectivesScale * HUDScaleY );
	else
		XL2 += 40.f*HUDScaleX;
	...
	for (i=0; i<6; i++)
	{
		if ( Level.ObjectiveInfos[i].Text != "" )
		{
			...
			C.DrawTextDropShadowed( Level.ObjectiveInfos[i].Text,,,, ObjectivesScale * HUDScaleX, ObjectivesScale * HUDScaleY );
	...
	if ( j == 0 )
	{
		...
		C.DrawTextDropShadowed( "No Objectives Available",,,, ObjectivesScale * HUDScaleX, ObjectivesScale * HUDScaleY );
	}
...
simulated function DrawCrossHair( canvas C, int X, int Y)
{
	...
	// TODO: Crosshair customizable custom player-chosen color custom color color
	XScale = 1.0 * HUDScale;
	XScale += PlayerPawn(Owner).WeapFireGrow;
	...
	{
		C.Style = Style;
		C.DrawColor = WhiteColor; // maybe a different color e.g. invert current hud color? -x
	}
	else
	{
		C.Style = Style;
...
simulated function string GetTypingPrompt()
{
	...
	local Console Console;
 	...
 	Console = PlayerOwner.Player.Console;
        Pos = Console.GetTypedStrPos();
	TypingPrompt = ">"@Left(Console.TypedStr, Pos);
	...
	if (CursorTime >= 0.3)
	{
        ...
        if ( Len(Console.TypedStr) != Pos ) 
          Temp = Mid(Console.TypedStr, Pos);

	if (TypingCursor)
	  return TypingPrompt$"_"$Temp;
        else
          return TypingPrompt$" "$Temp;      
}
...
simulated function DrawSOSMessages(canvas C)
{
	...
	// Still needs some adjustment, DrawLookAtInformation is good reference.
	X = 300 * HUDScaleX;
	Y = IndexTop + 32 * HUDScaleY;

	C.Style = ERenderStyle.STY_Translucent;

	if (SOSMessage.Text1 != "" && SOSMessage.Text2 != "") 
	{
		MiscBarOut = true;
		C.Font = SOSMessage.Font;

		if (SOSMessage.DoneDrawing && (Level.TimeSeconds - SOSMessage.DoneTime > 5.0))
		{
		...
		else if ( SOSMessage.DoneDrawing )
		{
			// Fading away...
			fadeAmount = (Level.TimeSeconds - SOSMessage.DoneTime) / 5.0;
			fadeAmount = 1.0 - fadeAmount;
			C.DrawColor.R = SOSMessage.TextColor.R * fadeAmount;
			C.DrawColor.G = SOSMessage.TextColor.G * fadeAmount;
			C.DrawColor.B = SOSMessage.TextColor.B * fadeAmount;

			C.SetPos(X, Y);
			C.DrawText(SOSMessage.Text1 );
			C.TextSize(SOSMessage.Text1, XL, YL);
			C.SetPos(X, Y + (5*HUDScaleY) + YL);
			C.DrawText( SOSMessage.Text2,,,, 0.75, 0.75 );
		}
		else
		{
			// Drawing in...
			FadeTime = Level.TimeSeconds - SOSMessage.Time;
			FadeTime *= 3;
			X2 = 0; Y2 = 0;
			S = SOSMessage.Text1;
			for ( FadeChar = 0; FadeTime - (FadeChar * 0.1) > 0.0; FadeChar++ )
			{
				...
				if (Len(SOSMessage.Text1) > Len(SOSMessage.Text2))
				{
					if ((FadeChar == Len(S) - 1) && (255 * (1.0 - FadeCount) < 0))
					{
						SOSMessage.DoneDrawing = true;
						SOSMessage.DoneTime = Level.TimeSeconds;
					}
				}

				C.DrawColor.R = Min(255 * (0.0 + FadeCount), SOSMessage.TextColor.R);
				C.DrawColor.G = Min(255 * (0.0 + FadeCount), SOSMessage.TextColor.G);
				C.DrawColor.B = Min(255 * (0.0 + FadeCount), SOSMessage.TextColor.B);
			...
			{
				FadeCount = FadeTime - (FadeChar * 0.1);
				if (Len(SOSMessage.Text2) > Len(SOSMessage.Text1))
				{
					if ((FadeChar == Len(S) - 1) && (255 * (1.0 - FadeCount) < 0))
					{
						SOSMessage.DoneDrawing = true;
				...
				C.SetPos( X+X2, Y+Y2+YL );
				C.DrawText( Mid(S, FadeChar, 1),,,, 0.75, 0.75 );
				C.TextSize( Left(S, FadeChar+1), XL, YL, 0.75, 0.75 );
...
simulated function RegisterSOSMessage(string EventMsg1, string EventMsg2, int X, int Y, int MessageIcon)
{
	PlayerOwner.PlaySound(SOSMessageSound, SLOT_Interface);

	SOSMessage.Text1		= EventMsg1;
	SOSMessage.Text2		= EventMsg2;
	//SOSMessage.X			= X;
	//SOSMessage.Y			= Y;
	SOSMessage.Font			= font'HUDFont'; //font'HUDFont';
	SOSMessage.TextColor	= TextColor;
	SOSMessage.Time			= Level.TimeSeconds;
	//SOSMessage.MessageIcon	= MessageIcon;
	SOSMessage.DoneDrawing	= false;
}
...
{
    bHideCrosshair = !bHideCrosshair;
    SaveConfig();
}

exec function ToggleLookAt()
{
    bHideLookAt = !bHideLookAt;
    SaveConfig();
}

// -x
exec function SetHudScale(int scale)
{
	default.HUDScale = 1.0 + ((scale / 8.0) * 0.5);
}
...
defaultproperties
{
    ...
    RootIndexTop=682.0
	//RootIndexTop=1080.0 * (682.0 / 768.0) // redundant?
	//IndexAdjust=20.0
	IndexAdjust=0.0
	BarOffset=0.0
    MaxIndexItems=12
