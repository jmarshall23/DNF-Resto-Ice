...
function bool ActivateMachine(int MachineIndex, optional Pawn Instigator)
{
	...
	// FairFriend: Can't activate slot machine if carrying an object
	if ( Instigator.IsA('DukePlayer') && !(DukePlayer(Instigator).CarriedDecoration == None ))
		return false;
...
{
	// FairFriend: Can't add bet to slot machine if carrying an object
	if ( Instigator.IsA('DukePlayer') && !(DukePlayer(Instigator).CarriedDecoration == None ))
		return false;
