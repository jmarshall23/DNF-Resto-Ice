...
state ShieldUp
{
	...
	simulated function BeginState()
	{
		...
		// Ch0wW: UGLY FIX for infinite flamethrower with the shield
		if (PlayerPawn(Owner).Weapon.IsA('Flamethrower') && PlayerPawn(Owner).Weapon.GetStateName() == 'Firing')
			return;	

		// If weapon is in a reloading state, disallow bringing up the shield.
		if (PlayerPawn(Owner).Weapon.GetStateName() == 'Reloading')
			return;	
