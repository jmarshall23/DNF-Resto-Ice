...
static DWORD GConversions[CPT_MAX][CPT_MAX] =
{
			...
			/*   --------  ------------------  ------------------  ----------------  -------------------  -----------------  ---------------  -------------------  ---------------  ---------------  ---------------- -------------------  -------------------- -----------------   */
/* None     */ { EX_Max,   EX_Max,             EX_Max,             EX_Max,           EX_Max,              EX_Max,            EX_Max,          EX_Max,              EX_Max,          EX_Max,          EX_Max,          EX_Max,              EX_Max,              EX_Max,             },
/* Byte     */ { EX_Max,   EX_Max,             EX_IntToByte|TAC,   EX_BoolToByte,	 EX_FloatToByte|TAC,  EX_Max,            EX_Max,          EX_Max,              EX_Max,          EX_Max,          EX_Max,          EX_Max,              EX_Max,              EX_StringToByte,    },
/* Int      */ { EX_Max,   EX_ByteToInt|AC,    EX_Max,             EX_BoolToInt,     EX_FloatToInt|TAC,   EX_Max,            EX_Max,          EX_Max,              EX_Max,          EX_Max,          EX_Max,          EX_Max,              EX_Max,              EX_StringToInt,     },
/* Bool     */ { EX_Max,   EX_ByteToBool,	   EX_IntToBool,       EX_Max,           EX_FloatToBool,      EX_ObjectToBool,   EX_NameToBool,   EX_Max,              EX_Max,          EX_Max,          EX_Max,          EX_VectorToBool,     EX_RotatorToBool,    EX_StringToBool,    },
/* Float    */ { EX_Max,   EX_ByteToFloat|AC,  EX_IntToFloat|AC,   EX_BoolToFloat,   EX_Max,              EX_Max,            EX_Max,          EX_Max,	           EX_Max,          EX_Max,          EX_Max,          EX_Max,              EX_Max,              EX_StringToFloat,   },
/* Object   */ { EX_Max,   EX_Max,             EX_Max,             EX_Max,           EX_Max,              EX_Max,            EX_Max,          EX_Max,              EX_Max,          EX_Max,          EX_Max,          EX_Max,              EX_Max,              EX_Max,             },
/* Name     */ { EX_Max,   EX_Max,             EX_Max,             EX_Max,           EX_Max,              EX_Max,            EX_Max,          EX_Max,              EX_Max,          EX_Max,          EX_Max,          EX_Max,              EX_Max,              EX_Max,             },
/*          */ { EX_Max,   EX_Max,             EX_Max,             EX_Max,           EX_Max,              EX_Max,            EX_Max,          EX_Max,              EX_Max,          EX_Max,          EX_Max,          EX_Max,              EX_Max,              EX_Max,             },
/*          */ { EX_Max,   EX_Max,             EX_Max,             EX_Max,           EX_Max,              EX_Max,            EX_Max,          EX_Max,              EX_Max,          EX_Max,          EX_Max,          EX_Max,              EX_Max,              EX_Max,             },
/*          */ { EX_Max,   EX_Max,             EX_Max,             EX_Max,           EX_Max,              EX_Max,            EX_Max,          EX_Max,              EX_Max,          EX_Max,          EX_Max,          EX_Max,              EX_Max,              EX_Max,             },
/* Struct   */ { EX_Max,   EX_Max,             EX_Max,             EX_Max,           EX_Max,              EX_Max,            EX_Max,          EX_Max,              EX_Max,          EX_Max,          EX_Max,          EX_Max,              EX_Max,              EX_Max,             },
/* Vector   */ { EX_Max,   EX_Max,             EX_Max,             EX_Max,           EX_Max,              EX_Max,            EX_Max,          EX_Max,			   EX_Max,          EX_Max,          EX_Max,          EX_Max,              EX_RotatorToVector,  EX_StringToVector,  },
/* Rotator  */ { EX_Max,   EX_Max,             EX_Max,             EX_Max,           EX_Max,              EX_Max,            EX_Max,          EX_Max,			   EX_Max,          EX_Max,          EX_Max,          EX_VectorToRotator,  EX_Max,              EX_StringToRotator, },
/* String   */ { EX_Max,   EX_ByteToString,    EX_IntToString,     EX_BoolToString,  EX_FloatToString,    EX_ObjectToString, EX_NameToString, EX_Max,			   EX_Max,          EX_Max,          EX_Max,          EX_VectorToString,   EX_RotatorToString,  EX_Max,             },
};
#undef AC
#undef TAC
...
{
	//BYTE B = Size;
	_WORD B = Size; // CDH: Changed to 16 bits because of unacceptable restriction on context expression size
...
void FScriptCompiler::EmitLet( const FPropertyBase& Type, const TCHAR *Tag )
{
	...
	if( Type.ArrayDim != 1 )
		appThrowf( TEXT("Can only assign individual booleans, not boolean arrays") );
...
INT FScriptCompiler::GetToken( FToken& Token, const FPropertyBase* Hint, INT NoConsts )
{
	...
	if( (c>='A' && c<='Z') || (c>='a' && c<='z') || (c=='_') )
	{
		...
		/* CDH: commented out, replaced with functions so we can actually use variables (whoopie)
...
INT FScriptCompiler::GetRawToken( FToken& Token )
{
	...
	while( !IsEOL(c) )
	{
		...
		Temp[Length++] = c;
		if( Length >= MAX_STRING_CONST_SIZE )
			appThrowf( TEXT("Identifier exceeds maximum of %i characters"), NAME_SIZE );
		c = GetChar(1);
	}
	UngetChar();
...
//
INT FScriptCompiler::GetConstInt( INT& Result, const TCHAR* Tag )
{
	FToken Token;
	if( GetToken(Token) ) 
	{
...
UStruct* FScriptCompiler::CompileStruct( UStruct* Scope )
{
	...
	// Unrealscript syntax: "struct native MyStruct { var() int whatever; ... };"
	UBOOL IsNative = 0;
	...
	if( MatchIdentifier(NAME_Expands) || MatchIdentifier(NAME_Extends) )
	{
		FToken ParentName;
		if( !GetIdentifier( ParentName ) )
			appThrowf( TEXT("'struct': Missing parent struct after 'Expands'") );
		BaseStruct = Cast<UStruct>( FindField( Scope, ParentName.Identifier, UStruct::StaticClass(), TEXT("'expands'") ) );
		if( !BaseStruct )
			appThrowf( TEXT("'expands': Can't find base struct '%s'"), ParentName.Identifier );
	}
...
)
{
	...
	else if( Token.Matches(NAME_Static) )
	{
		...
		AltFieldClass = UProperty::StaticClass(); // CDH
	...
	// CDH...
	else if( (Token.Matches(NAME_Vect) || Token.Matches(NAME_Rot)) && PeekSymbol(TEXT("(")) )
	{
		FieldClass = UFunction::StaticClass();
		Scope = UObject::StaticClass();
		Static = 1;
	}
	// ...CDH
	...
	// CDH...
	if (!Field && AltFieldClass)
		Field = FindField( Scope, Token.Identifier, AltFieldClass );
	// ...CDH
	...
	else if( Field && Cast<UProperty>(Field) )
	{
		...
		if( Default && Property->GetOuter()->GetClass()!=UClass::StaticClass() )
			appThrowf( TEXT("You can't access the default value of local variables") );
		if( !IsConcrete && !Default && !Static && Property->GetOuter()->GetClass()!=UFunction::StaticClass() ) // CDH static check
		...
		//if( !Default && !Static ) /*!! Allow writing to defaults (fix for Brandon) */ // CDH static check
			ResultType.PropertyFlags |= CPF_OutParm;
...
)
{
	...
	else if( Token.TokenName==NAME_New )
	{
		...
		if( Going )
		{
			CompileExpr( FPropertyBase(CPT_Name), TEXT("'new' name") );
			Going = MatchSymbol(TEXT(","));
		}
		else Writer << EX_Nothing;
...
void FScriptCompiler::CompileDirective()
{
	...
	else if( Directive.Matches(TEXT("Call")) || Directive.Matches(TEXT("AlwaysCall")) )
	{
		...
		{
			UTextBuffer* Text = ImportObject<UTextBuffer>( UObject::GetTransientPackage(), NAME_None, 0, Identifier.String, NULL, NULL, NULL, Warn );
			if( Text )
			{
...
)
{
	...
	UClass* TempClass;
	UBOOL IsVariable = 0;
	FName TempOptionNameA = NAME_None/*, TempOptionNameB = NAME_None*/;
	UObject* TempOptionObjectA = NULL/*, *TempOptionObjectB = NULL*/;
	...
	for( ; ; )
	{
		...
		// CDH...
		else if( Specifier.Matches(NAME_Static) )
		{
			//Flags	  |= CPF_Static;
			IsVariable = 1;
		}
		else if( Specifier.Matches(NAME_Unbound) )
		{
			//if (!(Class->GetFlags() & RF_Native))
			//	appThrowf(TEXT("'unbound' : This keyword is not valid for non-native classes"));
			//Flags     |= CPF_Unbound;
			IsVariable = 1;
		}
		else if( Specifier.Matches(NAME_State) )
		{
			Flags	  |= CPF_StateName;
			IsVariable = 1;
			TempOptionNameA = NAME_None;
			TempOptionObjectA = NULL;
			if (MatchSymbol(TEXT("(")))
			{
				FToken CategoryToken;
				if (!GetIdentifier(CategoryToken))
					appThrowf(TEXT("'state': Expecting category"));
				TempOptionNameA = FName(CategoryToken.Identifier);
				RequireSymbol(TEXT(")"), TEXT("'state name category'"));
			}
			if (MatchSymbol(TEXT("<")))
			{
				FToken ClassToken;
				if (!GetIdentifier(ClassToken))
					appThrowf(TEXT("'state': Expecting meta-class name"));
				TempOptionObjectA = FindObject<UClass>(ANY_PACKAGE, ClassToken.Identifier);
				if (!TempOptionObjectA)
					appThrowf(TEXT("'state': Class '%s' is an unrecognized class name"), ClassToken.Identifier);
				RequireSymbol(TEXT(">"), TEXT("'state name meta-class'"));
			}
		}
		// ...CDH
		else
	...
	else if( (TempClass = FindObject<UClass>( ANY_PACKAGE, VarType.Identifier ) ) != NULL )
	{
		...
		// CDH... external class enums/structs
		if( MatchSymbol(TEXT(".")) )
		{
			FToken ElemIdent;
			if( !GetIdentifier(ElemIdent) )
				appThrowf( TEXT("'%s': Missing class member type after '.'"), VarType.Identifier );
			if( (Field=FindField( TempClass, ElemIdent.Identifier ))!=NULL )
			{
				...
				if( Field->GetClass()==UEnum::StaticClass() )
					...
					appThrowf( TEXT("Unrecognized type '%s' within '%s'"), ElemIdent.Identifier, VarType.Identifier );
			}
			else
				appThrowf( TEXT("Unrecognized type '%s' within '%s'"), ElemIdent.Identifier, VarType.Identifier );
		}
	...
	// CDH...
	VarProperty.OptionNameA = VarProperty.OptionNameB = NAME_None;
	VarProperty.OptionObjectA = VarProperty.OptionObjectB = NULL;
	if (VarProperty.PropertyFlags & CPF_StateName)
	{
		if (VarProperty.Type != CPT_Name)
			appThrowf(TEXT("'state' is only allowed on properties of type 'name'"));
		VarProperty.OptionNameA = TempOptionNameA;
		VarProperty.OptionObjectA = TempOptionObjectA;
	}
	// ...CDH
...
)
{
	...
	if( !Skip )
	{
		...
		else if( VarProperty.Type==CPT_Name )
		{
			...
			if (VarProperty.PropertyFlags & CPF_StateName)
			{
				Cast<UNameProperty>(NewProperty)->NameOptionNameA = VarProperty.OptionNameA;
				Cast<UNameProperty>(NewProperty)->NameOptionObjectA = VarProperty.OptionObjectA;
			}
			// ...CDH
		}
	...
	// CDH... check for comment string	
	if (NewProperty && PeekSymbol(TEXT("?"))/*PeekIdentifier(NAME_Comment)*/)
	{
		FToken CommentToken;
		/*
		if (!GetIdentifier(CommentToken))
			appThrowf(TEXT("Expecting 'comment'"));
		*/
		if (!MatchSymbol(TEXT("?")))
			appThrowf(TEXT("Expecting '?'"));//appThrowf(TEXT("Expecting 'comment'"));

		if (!MatchSymbol(TEXT("(")))
			appThrowf(TEXT("Expecting '('"));
		if (!GetToken(CommentToken, NULL, 0))
			appThrowf(TEXT("Expecting string constant"));
		if (CommentToken.Type != CPT_String)
			appThrowf(TEXT("Expecting string constant"));
		NewProperty->CommentString = FString(CommentToken.String);
		NewProperty->PropertyFlags |= CPF_Comment;
		if (!MatchSymbol(TEXT(")")))
			appThrowf(TEXT("Expecting ')'"));
	}
	// ...CDH

	// JEP... (allow variable definitions override case)
	// This does NOT quite work, because it allow variable namess to override the case of unreal types like "Color"
	//	Need to find a solution to this problem
#if 0
	//Warn->Logf( NAME_Warning, TEXT("Var Case Override: %s"), *VarToken.TokenName);

	if (VarToken.TokenName != NAME_None)
	{
		FNameEntry *NameEntry = (FNameEntry*)VarToken.TokenName.GetEntry(VarToken.TokenName.GetIndex());

		if (NameEntry)
		{
			// Both strings should be the same length
			check(appStrlen(NameEntry->Name) == appStrlen(VarToken.Identifier));
			// Let script tokens override the case in FNames
			appStrcpy(NameEntry->Name, VarToken.Identifier);
		}
	}
#endif
	// ...JEP
...
INT FScriptCompiler::CompileDeclaration( FToken& Token, UBOOL& NeedSemicolon )
{
	if( Token.Matches(NAME_Class) && (TopNest->Allow & ALLOW_Class) )
	{
		...
		const TCHAR *TheClassName=Class->GetName();
		if(appStrstr(TheClassName,TEXT("___")))
		//if(appIsDigit(TheClassName[appStrlen(TheClassName)-1]))
			debugf(TEXT("********** Warning Class: \"%s\" has the ___ pattern, this is invalid, please change it immedietely."),TheClassName);
		...
		for( ; ; )
		{
			...
			else if( Token.Matches(NAME_NoUserCreate) )
			{
				// Can't create objects of this class in the editor.
				Class->ClassFlags |= CLASS_NoUserCreate;
			}
			...
			// CDH...
			else if( Token.Matches(NAME_Obsolete) )
			{
				Class->ClassFlags |= CLASS_Obsolete;
			}
			// ...CDH
			else
			{
		...
		// CDH...
		if (Class->GetOuter()->GetFName() == FName(TEXT("Botpack")))
			Class->ClassFlags |= CLASS_Obsolete;
		if (Class->GetOuter()->GetFName() == FName(TEXT("UnrealShare")))
			Class->ClassFlags |= CLASS_Obsolete;
		if (Class->GetOuter()->GetFName() == FName(TEXT("UnrealI")))
			Class->ClassFlags |= CLASS_Obsolete;
		// ...CDH
		...
		Class->AddDependency( Class );
		if( Class->GetSuperClass() )
			Class->AddDependency( Class->GetSuperClass() );

		// Copy properties from parent class.
		Class->Defaults[CPD_Normal].Empty();

		if( Class->GetSuperClass() )
		{
			Class->SetPropertiesSize( Class->GetSuperClass()->GetPropertiesSize() );
			Class->Defaults[CPD_Normal] = Class->GetSuperClass()->Defaults[CPD_Normal];
		}
	...
	|| (Token.Matches(NAME_Static) && (TopNest->Allow & ALLOW_Function) )
	...
	else if( Token.Matches(NAME_Var) || Token.Matches(NAME_Local) )
	{
		...
		if( (OriginalProperty.PropertyFlags & (CPF_Transient|CPF_Native)) && TopNest->NestType!=NEST_Class )
			appThrowf( TEXT("Local variables may not be transient or native") );
		if( OriginalProperty.PropertyFlags & CPF_ParmFlags )
			appThrowf( TEXT("Illegal type modifiers in variable") );
		...
		{
			FPropertyBase Property = OriginalProperty;
			GetVarNameAndDim( TopNode, Property, ObjectFlags, 0, 0, NULL, TEXT("Variable declaration"), EdCategory, 0 );
		} while( MatchSymbol(TEXT(",")) );
	}
	...
	||	Token.Matches(NAME_Simulated) )
	{
		...
		for( ;; )
		{
			if( Token.Matches(NAME_State) )
			{
				...
				if( MatchSymbol(TEXT("(")) )
				{
					...
					FToken CategoryToken;
					if (GetIdentifier(CategoryToken))
						StateCategory = FName(CategoryToken.Identifier);
					// ...CDH
		...
		if( MatchIdentifier(NAME_Expands) || MatchIdentifier(NAME_Extends) )
		{
			FToken ParentToken;
			if( ParentState )
				appThrowf( TEXT("'Expands' not allowed here: state '%s' overrides version in parent class"), NameToken.Identifier );
			...
			if( !ParentState )
				appThrowf( TEXT("'expands': Parent state '%s' not found"), ParentToken.Identifier );
		}
		...
		State->StructCategory = StateCategory;
		RequireSymbol( TEXT("{"), TEXT("'State'") );
		NeedSemicolon=0;
...
void FScriptCompiler::PostParse( UStruct* Node )
{
	...
	if( ThisClass )
        ThisClass->Defaults[CPD_Normal].AddZeroed( ThisClass->GetPropertiesSize() - ThisClass->Defaults[CPD_Normal].Num() );
...
void FScriptCompiler::CompileSecondPass( UStruct* Node )
{
	...
	if( Node==Class && Node->Line!=INDEX_NONE )
	{
		...
		while( 1 )
		{
			...
			// Check for multicast replication
			if( MatchIdentifier(NAME_ToAll) )
				FunctionFlags |= FUNC_Multicast;
			...
			do
			{
				...
				for( TFieldIterator<UProperty> It(Class); It && It.GetStruct()==Class; ++It )
				{
					if( It->GetFName()==VarName )
					{
						...
						// CDH...
						if( It->PropertyFlags & CPF_Static )
							appThrowf( TEXT("Variable '%s' is static; statics cannot be replicated"), *VarName );
						// ...CDH
				...
				if( !Found )
				{
					...
					for( TFieldIterator<UFunction> Function(Class); Function && Function.GetStruct()==Class; ++Function )
					{
						if( Function->GetFName()==VarName )
						{
							...
							/* reasonable? for( TFieldIterator<UProperty> It(*Function); It; ++It )
								if( (It->PropertyFlags & CPF_ReturnParm) && (q++>0) )
									appThrowf( TEXT("Functions that return a value may not be replicated") ); */
...
)
{
	...
	try
	{
		...
		if( Pass == 0 )
		{
            ...
            Class->Defaults[CPD_Normal].Shrink();
			check(Class->GetPropertiesSize()==Class->Defaults[CPD_Normal].Num());

			// Check native size.
			if
			(  (Class->GetFlags() & RF_Native)
			&&	OriginalPropertiesSize
			&&	Align(Class->Defaults[CPD_Normal].Num(),4)!=OriginalPropertiesSize
			&&	GCheckNatives )
			{
                Warn->Logf( NAME_Warning, TEXT("Native class %s size mismatch (script %i, C++ %i)"), Class->GetName(), Class->Defaults[CPD_Normal].Num(), OriginalPropertiesSize );
				GCheckNatives = 0;
			}

			// Set config and localized flags.
			for( TFieldIterator<UProperty> It(Class); It && It->GetOuter()==Class; ++It )
			{
			...
			// First-pass success.
			Class->GetDefaultObject()->InitClassDefaultObject( Class );
		}
		if( Pass==0 || GEditor->Bootstrapping )
		{
			// Update the default object.
			//!!weird order, may be incorrect. Initial pass only?
			Class->GetDefaultObject()->InitClassDefaultObject( Class );
			ImportProperties( Class, &Class->Defaults[CPD_Normal](0), (ULevel*)NULL, *Class->DefaultPropText, Class->GetOuter(), Warn );
		}
		Success = 1;
	}
	catch( TCHAR* ErrorMsg )
	{
		...
		if( GEditor->Bootstrapping )
		{
			...
			appErrorf( TEXT("Failed due to errors.") );
		}
...
static UBOOL ParseScripts( TArray<UClass*>& AllClasses, FScriptCompiler& Compiler, UClass* Class, UBOOL MakeAll, UBOOL Booting, UBOOL MakeSubclasses )
{
	...
	if (MakeSubclasses)
	{
		for( TArray<UClass*>::TIterator It(AllClasses); It; ++It )
			if( (*It)->GetSuperClass()==Class && !ParseScripts( AllClasses, Compiler, *It, MakeAll, Booting, MakeSubclasses ) )
				return 0;
	}
...
static void MarkChangedScripts(TArray<UClass*>& AllClasses, UBOOL MakeAll, UBOOL UseLog)
{	

	// if we're doing a full make, just mark everything
	if (MakeAll)
	{
		for (TArray<UClass*>::TIterator It(AllClasses); It; ++It)
		{
	...
	{
		// mark all classes whose script text has changed
		for (TArray<UClass*>::TIterator It(AllClasses); It; ++It)
		{
			// if we have no script text, skip it
			if (!(*It)->ScriptText)
				continue;
			
			// if the script text is the same as it was before, skip it
			if (((*It)->Dependencies.Num())
			 && ((*It)->GetScriptTextCRC() == (*It)->Dependencies(0).ScriptTextCRC)) // Dependencies(0) is always the class itself
				continue;
			
			// mark it
			(*It)->ClassFlags &= ~(CLASS_Compiled | CLASS_Parsed);
			if (UseLog)
				debugf(NAME_DevCompile, TEXT("   Marked: %s (script text changed)"), (*It)->GetName());
		}
	}

	{
		// mark all classes whose superclasses have had interface changes
		for (TArray<UClass*>::TIterator It(AllClasses); It; ++It)
		{
			// if we have no script text, skip it
			if (!(*It)->ScriptText)
			...
			for (UClass* Parent=(*It)->GetSuperClass(); Parent; Parent=Parent->GetSuperClass())
			{
				if (Parent->ScriptText && (!(Parent->ClassFlags & CLASS_Parsed)))
				{
					...
					(*It)->ClassFlags &= ~(CLASS_Compiled | CLASS_Parsed);
					if (UseLog)
						debugf(NAME_DevCompile, TEXT("   Marked: %s (ancestor '%s' changed)"), (*It)->GetName(), Parent->GetName());
					break;
				}
			}
		}
	...
	{
		// mark all classes whose immediate dependencies have had interface changes
		for (TArray<UClass*>::TIterator It(AllClasses); It; ++It)
		{
			// if we have no script text, skip it
			if (!(*It)->ScriptText)
			...
			for (INT i=1; i<(*It)->Dependencies.Num(); i++)
			{
				...
				{
					// mark it
					//(*It)->ClassFlags &= ~CLASS_Compiled; // CDH: This tends to be unsafe, flagging only compilation like this...
					(*It)->ClassFlags &= ~(CLASS_Parsed|CLASS_Compiled);
					if (UseLog)
						debugf(NAME_DevCompile, TEXT("   Marked: %s (dependency '%s' changed)"), (*It)->GetName(), (*It)->Dependencies(i).Class->GetName());
				}
			}
		}
	}
...
static UBOOL BuildScripts(UClass* BaseClass, TArray<UClass*>& AllClasses, FFeedbackContext* Warn, UTransactor* Transactor, UBOOL MakeAll, UBOOL Booting, UBOOL MakeSubclasses)
{
	FMemMark Mark(GMem);
	FTransactionBase* Transaction = Transactor ? Transactor->CreateInternalTransaction() : NULL;
	FScriptCompiler	Compiler( Warn );

	// Mark all classes known to be changed
	Compiler.ShowDep = ParseParam( appCmdLine(), TEXT("SHOWDEP") );
	MarkChangedScripts(AllClasses, MakeAll, Compiler.ShowDep);
	...
	for( TArray<UClass*>::TIterator It(AllClasses); It; ++It )
	{
		...
		{
			if( (*It)->GetDefaultObject() && (*It)->DefaultPropText==TEXT("") )
			{
				FStringOutputDevice Ar;
                GEditor->ExportProperties( Ar, *It, &(*It)->Defaults[CPD_Normal](0), 0, (*It)->GetSuperClass(), (*It)->GetSuperClass() ? (BYTE*)(*It)->GetSuperClass()->GetDefaultObject() : NULL );
	...
	// Do compiling.
	//Compiler.ShowDep = ParseParam( appCmdLine(), TEXT("SHOWDEP") ); // CDH
	Compiler.StatementsCompiled	= 0;
	Compiler.LinesCompiled		= 0;
	Compiler.ErrorText			= GEditor->Results;
	...
	for( TArray<UClass*>::TIterator It2(AllClasses); It2; ++It2 )
	{
		// Cleanup all exported property text.
		(*It2)->DefaultPropText=TEXT("");
	}
	if( Transaction )
		delete Transaction;
	Mark.Pop();
...
UBOOL UEditorEngine::MakeScripts( UClass* BaseClass, FFeedbackContext* Warn, UBOOL MakeAll, UBOOL Booting, UBOOL MakeSubclasses )
{
	...
	if (!BuildScripts(BaseClass, AllClasses, Warn, Trans, MakeAll, Booting, MakeSubclasses))
		return(0);
