/*=============================================================================
	...
	-fix every damn thing about this broken ass unstable ass editor before i rip my hair out of my scalp
...
// TODO: Figure out if this is really necesary
#ifdef _DEBUG
	#undef _T
#endif // _DEBUG

// Bugslayer
#ifdef BUGSLAYER_ENABLE
	#include "BugslayerUtil.h"
#endif // BUGSLAYER_ENABLE
...
DWORD IpcHandler(DWORD inSenderProcess, DWORD inMessage, DWORD inParamV, ANSICHAR* inParamS, void* inUserData)
{
	...
	case MACEDIT_IPC_OMSG_TEXREFUPDATE:
		debugf(TEXT("Cannibal: IPC TexRefUpdate"));
		if (MeshInst && MeshInst->Mac)
		{
			// map the configuration's textures
			for (DWORD i=0;i<MeshInst->Mac->mSurfaces.GetCount();i++)
...
INT WINAPI WinMain( HINSTANCE hInInstance, HINSTANCE hPrevInstance, char* InCmdLine, INT nCmdShow )
{
...
#ifndef _DEBUG
		InitSplash(TEXT("EdSplash.bmp"),IDDIALOG_Splash);
#endif
	// Play a welcome sound.
	if(!isExcusedFromAnnoyingSound())
		PlaySoundW( L"EditorRes\\DEStartup.wav", NULL, SND_ASYNC | SND_FILENAME | SND_NODEFAULT | SND_NOWAIT );
	...
	// Play a leaving sound.
	if(!isExcusedFromAnnoyingSound())
		PlaySoundW( L"EditorRes\\DEShutdown.wav", NULL, SND_ASYNC | SND_FILENAME | SND_NODEFAULT | SND_NOWAIT );
