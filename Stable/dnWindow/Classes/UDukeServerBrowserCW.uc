...
var		UDukeServerListFactory		Factories[10];
var     string                      serverListFactoryType;  // Type of server list factory to create 
var     config string               serverListFactoryTypes[10];  // Type of server list factory to create 
var     int                         serverListQueryDone[10];    // Set when the current query is completed
...
function QueryFinished( UDukeServerListFactory Fact, bool bSuccess, optional string ErrorMsg )
{
	...
	for(i=0;i<10;i++)
	{
		...
		{
			if(Factories[i] == Fact)
				serverListQueryDone[i] = 1;
				
			if(serverListQueryDone[i] == 0)
				bDone = False;
		}
	}
...
{   
	...
	// [JM] To-Do: Figure out why the fuck the ports aren't being let go after pinging.
	for(i=0;i<10;i++)
	{
		if(serverListFactoryType == "" || serverListFactoryTypes[i] == "")
		{
			Log( "Blank factory type!" );
			continue;
		}

		if(serverListFactoryType == "dnWindow.UDukeLocalFact")
		{
			Factories[0] = UDukeServerListFactory( BuildObjectWithProperties( serverListFactoryType ) );
			bLANOnly = true;
		}
		else
			Factories[i] = UDukeServerListFactory( BuildObjectWithProperties( serverListFactoryTypes[i] ) );

		if ( Factories[i] == None )
		{
			Log( "Could not create ServerFactory" @ serverListFactoryTypes[i] );
			return;
		}
...
defaultproperties
{
	...
	serverListFactoryTypes(0)="dnWindow.UDukeGSpyFact,MasterServerAddress=shadowmavericks.com,MasterServerTCPPort=28900,Region=0,GameName=dnf"
	serverListFactoryTypes(1)="dnWindow.UDukeGSpyFact,MasterServerAddress=master.333networks.com,MasterServerTCPPort=28900,Region=0,GameName=dnf"
}
