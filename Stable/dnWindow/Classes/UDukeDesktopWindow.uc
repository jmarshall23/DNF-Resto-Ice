...
var globalconfig float	BackgroundRatio;
var globalconfig bool	BackgroundFillToWidth;
...
var int iSaveGameIconIndex;						// FairFriend: Index of the save game icon to access arraySubIcons
var bool bGameSubIconsDecremented;				// FairFriend: True if the positions of the game sub icons have been decremented
...
function Created() 
{
	...
	// FairFriend: At first the positions of the sub icons 
	// have not been changed yet
	bGameSubIconsDecremented = false;
...
function LoadArt()
{
	...
	*/
	...
	// FairFriend: Load the texture of the window with the exit button only. 
	LookAndFeel.Active4 = LoadTexture( "menu_window4bc" );
...
function ResolutionChanged(float W, float H)
{
	...
	// FairFriend: Fix misplacement of the game sub-icons
	bGameSubIconsDecremented = false;
	ChangeGameSubIconsPosition();
}
...
// FairFriend: Increment or decrement the sub-icons of the Game string
// depending on whether the save game icon is visible or not
function ChangeGameSubIconsPosition()
{
	local int i;
	
	for ( i = iSaveGameIconIndex; i < ICON_Game + SUB_ICON_MAX; i++) 
	{
		if (arraySubIcons[i] != None)
		{
			if ( !arraySubIcons[iSaveGameIconIndex].bWindowVisible && !bGameSubIconsDecremented)
				arraySubIcons[i].WinLeft -= (arraySubIcons[i].UpTexture.USize + iIconOffsetX)*WinScaleX;
			else if (arraySubIcons[iSaveGameIconIndex].bWindowVisible && bGameSubIconsDecremented)
				arraySubIcons[i].WinLeft += (arraySubIcons[i].UpTexture.USize + iIconOffsetX)*WinScaleX;
		}
	}
	
	if ( !arraySubIcons[iSaveGameIconIndex].bWindowVisible && !bGameSubIconsDecremented) 	
		bGameSubIconsDecremented = true;
	else if (arraySubIcons[iSaveGameIconIndex].bWindowVisible && bGameSubIconsDecremented)
		bGameSubIconsDecremented = false;
}
...
function ToggleButtonsState( int iIconToChange, bool bSelected )
{
	...
	// FairFriend: Are we in the main menu?
	local bool bMainMenu;
	local string sMapName;
	sMapName = GetPlayerOwner().GetURLMap();
	if ( sMapName == "Entry.dnf" )
		bMainMenu = true;
	else	
		bMainMenu = false;
	...
	for ( iEnd = i + SUB_ICON_MAX; i < iEnd; i++ )
	{
		if ( arraySubIcons[i] != None )
		{
			// FairFriend: Show the icon if:
			// 1) The icon is selected...
			// 2) ...AND the icon is not visible already (in which case it should be hidden)...
			// 3) ...AND tt's not the save game icon unless we are not in the main menu
			
			if ( bSelected && !arraySubIcons[i].bWindowVisible && (i != iSaveGameIconIndex || !bMainMenu))	
				arraySubIcons[i].ShowWindow();
			else 
				arraySubIcons[i].HideWindow();
				
			if ( i == iSaveGameIconIndex )
				ChangeGameSubIconsPosition();
		}	
	}	
}
...
{
	local float PieceWidth, PieceHeight, BaseWidth, BaseHeight, X, Y, XOffset, YOffset, FillScale;
	local int i, j, k;
	...
	if(BackgroundRatio > 0.0)
	{
		BaseWidth = WinHeight*BackgroundRatio;

		if(BackgroundFillToWidth)
		{
			if(WinWidth > BaseWidth)
			{
				FillScale = WinWidth / BaseWidth;
				BaseWidth = WinWidth;
				BaseHeight = BaseHeight*FillScale;
			}
		}

		XOffset = (WinWidth-BaseWidth)/2;
		YOffset = (WinHeight-BaseHeight)/2;
	}

	PieceWidth = BaseWidth / BackgroundTileCountH;
	PieceHeight= BaseHeight / BackgroundTileCountV;
	...
	for ( i=0; i<BackgroundTileCountH; i++ )
	{
		...
		for ( j=0; j<BackgroundTileCountV; j++ )
		{
			DrawStretchedTextureSegment( C, X + XOffset, Y + YOffset, PieceWidth, PieceHeight, 0, 0, Backdrop[k].USize, Backdrop[k].VSize, Backdrop[k], 1.f, true, true );
			Y += PieceHeight;
			k++;
		}
		X += PieceWidth;
...
function ShowProfileWindow(optional bool Force)
{
	...
	// FairFriend: Prevents being stuck when creating profile for the first time.
	if (bIconsHidden)
		Root.DontCloseOnEscape = true;
}
...
// FairFriend: When menu is hidden we also hide all 
// the sub-icons, so when it is opened again they are
...
// FairFriend: Hide all the sub-icons
function HideSubIcons()
{
	local int i;
