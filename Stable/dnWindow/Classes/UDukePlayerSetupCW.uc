...
function TestHitSound()
{
	...
	{
		PlayerOwner.PlaySound( DukePlayer( PlayerOwner ).HitNotificationSound );
	}
}
...
{
	IterateThings( "dnGame.DukePlayer", "Sound", HitSoundCombo, "" );   
	HitSoundCombo.SetSelectedIndex( 0 );
}
...
function UseSelected()
{
	...
	{
		//DukePlayer( GetPlayerOwner() ).HitNotificationSound = Sound( DynamicLoadObject( HitSoundCombo.GetValue2(), class'Sound' ) );
		DukePlayer( GetPlayerOwner() ).HitNotificationSoundName = HitSoundCombo.GetValue2();
	}
}
...
function LoadCurrent()
{
	...
	if ( DukePlayer( GetPlayerOwner() ) != None )
	{
		hitSndIndex = HitSoundCombo.FindItemIndex2(DukePlayer(GetPlayerOwner()).HitNotificationSoundName, false);
		HitSoundCombo.SetSelectedIndex( hitSndIndex );
	}
}
