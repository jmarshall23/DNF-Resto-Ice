...
// HUD Scale
var UWindowLabelControl ScaleLabel;
var UWindowHSliderControl ScaleSlider;
...
function Created()
{
	...
	// HUD Scale
	ScaleLabel = UWindowLabelControl(CreateControl(class'UWindowLabelControl', 1, 1, 1, 1));
	ScaleLabel.SetText(ScaleText);
	ScaleLabel.SetFont(F_Normal);
	ScaleLabel.Align = TA_Right;

	ScaleSlider = UWindowHSliderControl(CreateControl(class'UWindowHSliderControl', 1, 1, 1, 1));
	ScaleSlider.bNoSlidingNotify = True;
	ScaleSlider.SetRange(1, 8, 1);
	ScaleSlider.SetHelpText(ScaleHelp);
	ScaleSlider.SetFont(F_Normal);
	ScaleSlider.Align = TA_Right;
...
function LoadAvailableSettings()
{
	...
	ScaleSlider.SetValue(((class'DukeHUD'.default.HUDScale - 1.0) / 0.5) * 8);
...
function BeforePaint( Canvas C, float X, float Y )
{
	...
	// HUD Scale
	ScaleSlider.SetSize(CenterWidth, ScaleSlider.WinHeight);
	ScaleSlider.SliderWidth = 150;
	ScaleSlider.WinLeft = CColRight;
	ScaleSlider.WinTop = MouseSlider.WinTop + MouseSlider.WinHeight + ControlOffset;

	ScaleLabel.AutoSize(C);
	ScaleLabel.WinLeft = CColLeft - ScaleLabel.WinWidth;
	ScaleLabel.WinTop = ScaleSlider.WinTop + 4;

	// for whatever reason the window height still doesnt adjust, and the bottom is slightly cut off -x

	// GUI Red
	GUIRedSlider.SetSize( CenterWidth, GUIRedSlider.WinHeight );
	GUIRedSlider.SliderWidth = 150;
	GUIRedSlider.WinLeft = CColRight;
	GUIRedSlider.WinTop = ScaleSlider.WinTop + ScaleSlider.WinHeight + ControlOffset;
...
function BackgroundChanged()
{
	...
	local bool bIsSmack, bFillToWidth;
	...
	if ( Pos > -1 )
	{
		...
		if ( Pos > -1 )
		{
			...
			if ( Pos > -1 )
			{
				aspectRatio = float(Left( RestOfData, Pos ));
				RestOfData = Right( RestOfData, Len(RestOfData) - Pos );
			}
			else
			{
				aspectRatio = float(RestOfData);
				RestOfData = "";
			}
		}
		else
		{
			GetPlayerOwner().BroadcastMessage("Background ratio incorrectly defined, Aborting.");
			return;
		}

		Pos = InStr( ExtraData, "fill_width" );
		if ( Pos > -1 )
		{
			bFillToWidth = true;
		}
	...
	UDukeRootWindow(Root).Desktop.BackgroundRatio = aspectRatio;
	UDukeRootWindow(Root).Desktop.BackgroundFillToWidth = bFillToWidth;
	for ( i=0; i<12; i++ )
	{
...
{
	Class'DukeHUD'.default.HUDScale = 1.0 + ((ScaleSlider.Value / 8) * 0.5);
}
...
defaultproperties
{
	...
	ScaleText = "HUD Scale"
	ScaleHelp = "Tweak the scale of the HUD"
