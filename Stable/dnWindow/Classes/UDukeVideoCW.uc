...
// Fullscreen
var UWindowLabelControl FullscreenLabel;
var UWindowCheckbox CheckFullscreen;
...
function Created()
{
	...
	// Fullscreen
	FullscreenLabel = UWindowLabelControl(CreateControl(class'UWindowLabelControl', ControlLeft, ControlOffset, ControlWidth, 1));
	FullscreenLabel.SetText(FullscreenText);
	FullscreenLabel.SetFont(F_Normal);
	FullscreenLabel.Align = TA_Right;

	CheckFullscreen = UWindowCheckbox(CreateControl(class'UWindowCheckbox', CenterPos, ControlOffset, CenterWidth, 1));
	CheckFullscreen.bChecked = bool(GetPlayerOwner().ConsoleCommand("get ini:Engine.Engine.ViewportManager StartupFullscreen"));
	CheckFullscreen.SetHelpText(FullscreenHelp);
	CheckFullscreen.SetFont(F_Normal);
	CheckFullscreen.Align = TA_Right;
...
function BeforePaint( Canvas C, float X, float Y )
{
	...
	CheckFullscreen.SetSize( CenterWidth-90+16, CheckFullscreen.WinHeight );
	CheckFullscreen.WinLeft = CColRight;
	CheckFullscreen.WinTop = ResolutionCombo.WinTop + ResolutionCombo.WinHeight + ControlOffset + CheckFullscreen.GetHeightAdjust();
	
	FullscreenLabel.AutoSize( C );
	FullscreenLabel.WinLeft = CColLeft - FullscreenLabel.WinWidth;
	FullscreenLabel.WinTop = CheckFullscreen.WinTop + 10;	

	ColorDepthCombo.SetSize( 200, ColorDepthCombo.WinHeight );
	ColorDepthCombo.WinLeft = CColRight;
	ColorDepthCombo.WinTop = CheckFullscreen.WinTop + CheckFullscreen.WinHeight + ControlOffset + 8 + CheckFullscreen.GetHeightAdjust()*2;
...
{	
	GetPlayerOwner().ConsoleCommand("set ini:Engine.Engine.ViewportManager StartupFullscreen "$CheckFullscreen.bChecked);	
...
defaultproperties
{
	 ...
	 FullscreenHelp="Toggle fullscreen on and off."
