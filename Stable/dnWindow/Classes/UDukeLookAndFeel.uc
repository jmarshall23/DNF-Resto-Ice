...
{
	local Texture T, T3;
	// FairFriend: Texture of the window with the exit button only. 
	local Texture T4;
	...
	// FairFriend: Retrieve texture of window with exit button only. 
	T4 = W.GetLookAndFeelTexture4();

	// Draw the main frame.
	C.DrawColor = GetGUIColor(W);
	Real_DrawWindowFrame( W, C, T, T3, T4, false );
	...
	// FairFriend: For the glow effect we can use the same texture used 
	// 			   ...by the window with both buttons
	T4 = W.GetLookAndFeelGlowTexture();

	// Draw the frame glow.
	C.DrawColor = W.WhiteColor;
	Real_DrawWindowFrame( W, C, T, T3, T4, true );

	DrawTitleAndStatus( W, C );
}

function Real_DrawWindowFrame( UWindowFramedWindow W, Canvas C, Texture T, Texture T3, Texture T4, bool bGlow )
{
	local Region R;
	...
	if ( W.bMessageBoxFrame )
		UseT = T3;	// FairFriend: Texture with no exit/reset buttons.
	else if ( W.bExitButtonOnly )
		UseT = T4;	// FairFriend: Texture with only the exit button. 
	else
		UseT = T;	// FairFriend: Texture with all the buttons.
