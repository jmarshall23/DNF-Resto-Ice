...
U32 GlTexCache::init(void)
{
	...
	while(size_left)
	{
		...
		active.insert_after(current, entry);
		current = entry;
...
U32 GlTexCache::cache(GVidTex *tex)
{
	...
	// Raziel: Not 100% confident in the caching code fixes below, this wasn't compiling at all, it
	//  does now, but if there's a problem with the tool, it will most likely be caused by one of
	//  the two active.insert statements below:
	entry=get_avail();
	entry->set_texture(tex->get());
	entry->set_address(start_addr);
	entry->set_size(size_needed);
	
	// Raziel: This used to be an insert_before, but it makes sense in my head that we'd want to
	//  insert the cache entry after the 'current' one, which should be the last cache entry that's
	//  not invalidated yet.
	active.insert_after(active.get_cur(), entry);
	...
	if (size_left)
	{
		...
		// Raziel: Again, mostly an assumption, but I think any hole required in the address space
		//  would likely be after the entry we just added.
		active.insert_after(entry, hole);
	}
...
GVidTex::GVidTex(CC8 *Name,XImage* const &Image)
{
	if (Name)
	{
		name=Name;
