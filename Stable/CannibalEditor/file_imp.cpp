...
static void ParseUVList(chunk_t* chunk, void* limit)
{
	word numuvs = *((word*)chunk->data);
	void* uvs = (byte*)chunk->data + 2;
	...
	for (dword i = 0; i < numuvs; i++)
	{
		if (loadingUVs)
		{
			loadingUVs[num3dsuvs + i].x = *((float*)uvs);
			loadingUVs[num3dsuvs + i].y = *((float*)uvs + 1);
			loadingUVs[num3dsuvs + i].z = 0.0f;
		}
		uvs = (byte*)uvs + 8;
	}
...
static void ParseTriObject(chunk_t *chunk, void *limit)
{
	...
	while (ReadChunk(&subChunk, limit))
	{
		...
		if (subChunk.type == 0x4140)
			ParseUVList(&subChunk, (byte*)subChunk.data+subChunk.length);
	}
}
...
//    Public Code
//----------------------------------------------------------------------------
int FI_Load3DS(char *filename, int *numframes, int *numverts, vector_t **frameVerts,
			   int *numfaces, int **faces, char **frameNames, vector_t **frameUVs)
{
	byte *fbuffer, *fbend;
	FILE *fp;
	int len;
	...
	if (frameUVs && num3dsuvs > 0 && num3dsverts == num3dsuvs)
		loadingUVs = *frameUVs = ALLOC(vector_t, num3dsuvs);
	if (faces)
...
int FI_LoadMDL(char *filename, int *numframes, int *numverts, vector_t **frameVerts,
			   int *numfaces, int **faces, char **frameNames, vector_t** frameUVs)
{
	FILE *fp;
	char filebuf[_MAX_PATH];
...
int FI_LoadMD2(char *filename, int *numframes, int *numverts, vector_t **frameVerts,
			   int *numfaces, int **faces, char **frameNames, vector_t** frameUVs)
{
	FILE *fp;
	char filebuf[_MAX_PATH];
...
int FI_LoadMXB(char *filename, int *numframes, int *numverts, vector_t **frameVerts,
               int *numfaces, int **faces, char **frameNames, vector_t** frameUVs)
{
    FILE *fp;
    char filebuf[_MAX_PATH];
...
int FI_LoadGMA(char *filename, int *numframes, int *numverts, vector_t **frameVerts,
			   int *numfaces, int **faces, char **frameNames, vector_t** frameUVs)
{
	FILE *fp;
	char filebuf[_MAX_PATH];
...
int FI_LoadLWO(char *filename, int *numframes, int *numverts, vector_t **frameVerts,
			   int *numfaces, int **faces, char **frameNames, vector_t** frameUVs)
{
	FILE *fp;
	char filebuf[_MAX_PATH];
