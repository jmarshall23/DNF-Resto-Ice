...
typedef int (*frameImportFunc_t)(char *filename, int *numframes, int *numverts,
								vector_t **frameVerts, int *numfaces, int **faces,
								char **frameNames, vector_t **frameUVs);
...
//**
//****************************************************************************
int modelFrame_t::Import(int inNumVerts, vector_t *inVerts, int inNumTris, int *inTris, vector_t* inUVs)
{
	int i;
...
//		verts[i].mountIndex = 0;
	...
	if (inUVs && mdl->skins[0].tex)
	{
		// Raziel: CannibalEd enhancement: if we have a texture loaded and have a set of UVs, import them
		int width = mdl->skins[0].tex->width;
		int height = mdl->skins[0].tex->height;

		for (i = 0; i < numTris; i++)
		{
			int* vertexIndexes = inTris + (i * 3);
			baseTris[i].tverts[0].Set(inUVs[vertexIndexes[0]].x * width, height - (inUVs[vertexIndexes[0]].y * height), 0);
			baseTris[i].tverts[1].Set(inUVs[vertexIndexes[1]].x * width, height - (inUVs[vertexIndexes[1]].y * height), 0);
			baseTris[i].tverts[2].Set(inUVs[vertexIndexes[2]].x * width, height - (inUVs[vertexIndexes[2]].y * height), 0);
			baseTris[i].flags = BTF_INUSE;
			baseTris[i].skinIndex = 0;
		}

		CON->Printf("Texture coordinates imported from 3D model file");
	}
	else
	{
		// Raziel: Original 3DR behaviour requires that UVs be set up manually in CannibalEd
		for (i = 0; i < numTris; i++)
		{
		...
		if (inUVs)
		{
			CON->Printf("WARNING: Texture coordinates were not imported as\nno texture is loaded in row one of the skins tab.");
		}
	}
...
int model_t::ImportFrames(char *filename, boolean forceRestart)
{
	...
	for (fi=frameImports;fi->func;fi++)
	{
		...
		{
			if (!fi->func(filename, &numframes, &numverts, &frameVerts, &numtris, &frameTris, &frameNames, &frameUVs))
			{
				if (frameVerts)
					FREE(frameVerts);
				if (frameTris)
	...
	for (i=0;i<numframes;i++)
	{
		...
		if (!f->Import(numverts, frameVerts+(i*numverts), numtris, frameTris, frameUVs))
		{
			DeleteFrame(f);
		}
		else
