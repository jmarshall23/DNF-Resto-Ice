...
public:
	...
	~modelTrimesh_t() { if (meshTris) FREE(meshTris); if (mountPoints) FREE(mountPoints); }
...
public:
	...
	~modelFrame_t()
	{
		RemoveFromCache();
		if (verts) FREE(verts);
		if (baseTris) FREE(baseTris);
		if (frmdData) FREE(frmdData);
		if (vertNorms) FREE(vertNorms);
	}

	int Import(int inNumVerts, vector_t *inVerts, int inNumTris, int *inTris, vector_t* inUVs);
...
public:
	...
	seqTrigger_t *AddTrigger()
	{
		...
		trig->trigger = ALLOC(char, strlen(iStr)+1);
...
private:
	...
	boolean RecursiveCircularCheck(int mIndex);
...
public:
	...
	model_t *attachModel;
	boolean useAttachOrigin;

	modelMount_t()
	{
		v[0] = v[1] = v[2] = 0;
		barys[0] = barys[1] = barys[2] = 0.3333;
	...
	int UpdateTransform(float updateTime, boolean forceUpdate);
...
public:
	...
	{
		RecursiveMountToWorld(point, sys_curTime);
	}

	void WorldToMount(vector_t &point)
	{
		RecursiveWorldToMount(point, sys_curTime);
	}
};
//****************************************************************************
//**
...
public:
	...
	int ImportFrames(char *filename, boolean forceRestart); // import a frame or series of frames
	int SaveMDX(char *filename);
	int LoadMDX(char *filename, boolean useProjectData);
};
//****************************************************************************
//**
//**    END CLASS model_t
