...
// TODO: DNF removed this
#if UTGLR_DNF_BUILD
#define BLIT_OpenGL 0
#endif
...
UBOOL UOpenGLRenderDevice::Init(UViewport* InViewport, INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen) {
	...
	if (NumDevices == 0) {
		...
		if (!hModuleGlMain) {
			debugf(NAME_Init, (TCHAR*)"NoFindGL", appFromAnsi(GL_DLL));
			return 0;
		}
	...
	if (!SetRes(NewX, NewY, NewColorBytes, Fullscreen)) {
		return FailedInitf((TCHAR*)"ResFailed");
	}
...
void UOpenGLRenderDevice::Lock(FColor InFogColor, float InFogDensity, INT InFogDistance, FPlane InFlashScale, FPlane InFlashFog, FPlane ScreenClear, DWORD RenderLockFlags, BYTE* InHitData, INT* InHitSize) {
...
#if defined UTGLR_DX_BUILD || defined UTGLR_RUNE_BUILD || defined UTGLR_DNF_BUILD
...
void UOpenGLRenderDevice::DrawGouraudPolygon(FSceneNode* Frame, FTextureInfo& Info, FTransTexture** Pts, INT NumPts, DWORD PolyFlags, FSpanBuffer* Span, DWORD PolyFlagsEx) {
...
void UOpenGLRenderDevice::DrawTile(FSceneNode* Frame, FTextureInfo& Info, FLOAT X, FLOAT Y, FLOAT XL, FLOAT YL, FLOAT U, FLOAT V, FLOAT UL, FLOAT VL, class FSpanBuffer* Span, FLOAT Z, FPlane Color, FPlane Fog, DWORD PolyFlags, DWORD PolyFlagsEx, FLOAT alpha, FLOAT rot, FLOAT rotationOffsetX, FLOAT rotationOffsetY) {
...
void UOpenGLRenderDevice::ReadPixels(FColor* Pixels, UBOOL BackBuffer) {
...
#if UTGLR_UNREAL_227_BUILD || UTGLR_DNF_BUILD
		case TEXF_DXT3:
...
#if UTGLR_RUNE_BUILD || UTGLR_UNREAL_227_BUILD
